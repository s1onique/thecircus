...
let private newTempRepo () : string =
    let path =
        Path.Combine(Path.GetTempPath(), "circus-cp-test-" + System.Guid.NewGuid().ToString("n"))
    ...
    testList
        ...
        [ test "all CP-NN check ids are enumerable" {
          ...
          test "CP-01 detects a missing required file (negative mutation)" {
              ...
              Expect.isTrue
                  (List.exists (fun v -> v.Path.Contains "Dockerfile.backend") violations)
                  "missing file flagged"
          ...
          test "CP-22 detects missing numeric non-root user (negative mutation)" {
              ...
              Expect.isTrue
                  (List.exists (fun v -> v.Detail.Contains "numeric non-root") violations)
                  "missing numeric user flagged"
