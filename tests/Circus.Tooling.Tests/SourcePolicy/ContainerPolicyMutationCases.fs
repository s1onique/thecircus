...
let private baselineWorkflowOnly (root: string) : Result<unit, string> =
    (writeAndHash root ".github/workflows/harbor.yml" compliantHarbor)
    |> Result.bind (fun (_) ->
        (writeAndHash root ".dockerignore" compliantDockerignore)
        |> Result.bind (fun (_) -> Ok(())))

let private baselineBothWorkflows (root: string) : Result<unit, string> =
    (writeAndHash root ".github/workflows/harbor.yml" compliantHarbor)
    |> Result.bind (fun (_) ->
        (writeAndHash root ".github/workflows/harbor-build-image.yml" compliantReusable)
        |> Result.bind (fun (_) ->
            (writeAndHash root ".dockerignore" compliantDockerignore)
            |> Result.bind (fun (_) -> Ok(()))))
...
let private baselineWithCa (root: string) : Result<unit, string> =
    ...
    |> Result.bind (fun (_) ->
        (writeAndHash root ".github/scripts/install-spbnix-harbor-ca.sh" compliantCaScript)
        |> Result.bind (fun (_) ->
...
let private baselineWithPassword (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root ".github/workflows/harbor-build-image.yml" compliantReusableWithPassword)
        |> Result.bind (fun (_) -> Ok(())))

let private baselineWithMetadata (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root ".github/scripts/harbor-metadata.sh" compliantMetadata)
        |> Result.bind (fun (_) ->
...
let private baselineWithFrontend (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root "Dockerfile.frontend" compliantFrontend)
        |> Result.bind (fun (_) -> Ok(())))

let private baselineWithSmoke (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root "scripts/container-smoke.sh" compliantSmoke)
        |> Result.bind (fun (_) ->
...
let private baselineWithVerify (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root "scripts/ci/verify_build_image.sh" compliantVerify)
        |> Result.bind (fun (_) ->
            (makeExecutable root "scripts/ci/verify_build_image.sh")
            |> Result.bind (fun (_) ->
                (writeAndHash root "scripts/verify-published-image.sh" compliantVerifyPublished)
                |> Result.bind (fun (_) ->
...
let private baselineFullScriptSurface (root: string) : Result<unit, string> =
    (baselineBothWorkflows root)
    |> Result.bind (fun (_) ->
        (writeAndHash root "scripts/ci/build_image.sh" compliantBuildScript)
        |> Result.bind (fun (_) ->
            (makeExecutable root "scripts/ci/build_image.sh")
            |> Result.bind (fun (_) ->
                (writeAndHash root "scripts/ci/publish_image.sh" compliantPublishScript)
                |> Result.bind (fun (_) ->
                    ...
                    |> Result.bind (fun (_) ->
                        ...
                        |> Result.bind (fun (_) ->
                            (makeExecutable root "scripts/ci/verify_build_image.sh")
                            |> Result.bind (fun (_) ->
                                (writeAndHash root "scripts/ci/wire_buildx_builder.sh" compliantWireScript)
                                |> Result.bind (fun (_) ->
...
let private baselineFullSurfaceAndAcceptance (root: string) : Result<unit, string> =
    (baselineFullScriptSurface root)
    |> Result.bind (fun (_) ->
        (writeAndHash root "tests/ci/test_build_publish_shell.sh" compliantShellTest)
        |> Result.bind (fun (_) ->
            (makeExecutable root "tests/ci/test_build_publish_shell.sh")
            |> Result.bind (fun (_) ->
                (writeAndHash root "tests/ci/test_gate_summary_acceptance.sh" compliantAcceptanceTest)
                |> Result.bind (fun (_) ->
...
let mutationCases: MutationCase list =
    [ { Id = MutationCaseId.fromString "CP-04_workflow_triggers"
      ...
      { Id = MutationCaseId.fromString "CP-14_ca_secret"
        ...
        ApplyMutation = fun root -> replaceFile root ".github/scripts/install-spbnix-harbor-ca.sh" "echo no-ca\n" }
        ...
        ApplyMutation =
          fun root ->
              replaceFile
                  ...
                  ".github/workflows/harbor.yml"
                  (compliantHarbor
                      .Replace("cache_name: circus-backend", "cache_name: circus-shared")
      ...
      { Id = MutationCaseId.fromString "CP-16_publish_gating"
        ...
        ApplyMutation = fun root -> replaceFile root "scripts/ci/build_image.sh" "echo no-publish\n" }
      ...
      { Id = MutationCaseId.fromString "CP-17_cache_import_export"
        ...
        PrepareBaseline = baselineFullScriptSurface
        ApplyMutation = fun root -> replaceFile root "scripts/ci/build_image.sh" "docker build .\n" }
      ...
      { Id = MutationCaseId.fromString "CP-18_immutable_tag"
        ...
        ApplyMutation = fun root -> replaceFile root ".github/scripts/harbor-metadata.sh" "echo v0.1.0\n" }
      ...
      { Id = MutationCaseId.fromString "CP-19_latest_main_only"
        ...
        ApplyMutation =
          fun root ->
              replaceFile
                  root
                  ".github/scripts/harbor-metadata.sh"
                  "echo local-AAA\necho v0\necho 0\necho 0.0\necho 0\nif [ \"$GITHUB_REF\" = \"refs/heads/main\" ]; then echo latest; fi\nif [ \"$GITHUB_REF\" = \"refs/heads/main\" ]; then echo latest; fi\n" }
      ...
      { Id = MutationCaseId.fromString "CP-24_backend_smoke"
        ...
        ApplyMutation = fun root -> replaceFile root "scripts/container-smoke.sh" "echo no-smoke\n" }
      ...
      { Id = MutationCaseId.fromString "CP-25_digest_pull"
        ...
        ApplyMutation = fun root -> replaceFile root "scripts/ci/verify_build_image.sh" "docker pull IMAGE\n" }
      ...
      { Id = MutationCaseId.fromString "CP-27_github_output"
        ...
        PrepareBaseline = baselineFullScriptSurface
        ApplyMutation = fun root -> replaceFile root "scripts/ci/build_image.sh" "docker buildx build .\n" }
      ...
      { Id = MutationCaseId.fromString "CP-31_acceptance_marker"
        ...
        ApplyMutation = fun root -> replaceFile root "tests/ci/test_gate_summary_acceptance.sh" "echo incomplete\n" } ]
