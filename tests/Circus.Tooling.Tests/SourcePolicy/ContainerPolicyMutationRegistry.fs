...
module MutationCaseId =
    ...
    let compare (a: MutationCaseId) (b: MutationCaseId) : int = String.CompareOrdinal(value a, value b)
...
let defaultWorkspaceSeam: WorkspaceSeam =
    { CreateTempDir =
        fun () ->
            let path =
                Path.Combine(Path.GetTempPath(), "circus-cp-mut-" + Guid.NewGuid().ToString("n"))
      ...
      RunCheck =
        fun (id: string) (root: string) ->
            try
                Ok(runCheckById id root)
            with
            | CheckFailed msg -> Error(sprintf "check %s raised CheckFailed: %s" id msg)
            | ex -> Error(sprintf "check %s raised %s: %s" id (ex.GetType().Name) ex.Message) }
...
type MutationReceipt =
    ...
    member r.IsNonVacuous = r.HasObservableChange()
...
type RegistryExecutionFailure = InvalidRegistry of RegistryValidation

type MutationResults = Map<MutationCaseId, Result<MutationSuccess, MutationFailure>>

type RegistryOutcome = Result<MutationResults, RegistryExecutionFailure>



let private allProductionCheckIds: Set<string> = set CheckIds
...
let resolveContainedPath (root: string) (relativePath: string) : Result<string, string> =
    try
...
let buildReceipt (items: (string * Result<string * string, string>) list) : Result<MutationReceipt, string> =
...
/// Make a script executable in the workspace.  Routes the
/// target path through ``resolveContainedPath`` so a relative
/// path that escapes the workspace is rejected the same way
/// ``writeAndHash`` rejects it.
...
let writeAndHash (root: string) (rel: string) (newContent: string) : Result<string * string, string> =
    match resolveContainedPath root rel with
    | Error e -> Error e
    | Ok full ->
        try
            ...
            let beforeHash = if File.Exists full then sha256OfFile full else ""
    ...
    let rec walk (dir: string) : string list =
        ...
        if Directory.Exists dir then
            for f in Directory.GetFiles(dir) do
                let rel = f.Substring(root.Length).TrimStart(Path.DirectorySeparatorChar, '/')
                acc <- rel :: acc
...
let deriveChangedPaths (before: Map<string, string>) (after: Map<string, string>) : Set<string> =
    let allKeys =
        Set.union (before |> Map.keys |> Set.ofSeq) (after |> Map.keys |> Set.ofSeq)

    Set.filter (fun path -> Map.tryFind path before <> Map.tryFind path after) allKeys

let private combineFailures (primary: MutationFailure) (cleanupMsg: string) : MutationFailure =
    let primaryMsg =
        match primary with
...
let private executeCase (c: MutationCase) (seam: WorkspaceSeam) : Result<MutationSuccess, MutationFailure> =
    match seam.CreateTempDir() with
    | Error e -> Error(BaselinePreparationFailed e)
    | Ok root ->
            ...
            let baselineStep () : Result<unit, MutationFailure> =
                ...
                with ex ->
                    Error(
                        CaseExecutionFailed(
                            sprintf
                                "PrepareBaseline for %s threw %s: %s"
                ...
                | Ok baselineViolations ->
                    if not (List.isEmpty baselineViolations) then
                        ...
                        let mutationStep () : Result<MutationReceipt, MutationFailure> =
                            ...
                            with ex ->
                                Error(
                                    CaseExecutionFailed(
                                        sprintf
                                            "ApplyMutation for %s threw %s: %s"
                                            (MutationCaseId.value c.Id)
                                            (ex.GetType().Name)
                                            ex.Message
                                    )
                                )
                        // Phase C: snapshot, mutate exactly once, snapshot
                        // again.  The mutator must run only once.
                        ...
                        | Ok claimedReceipt ->
                            ...
                            let claimedChanged = Set.ofList claimedReceipt.ChangedPaths

                            if Set.isEmpty actualChanged then
                                Error(
                                    MutationWasVacuous(
                                        sprintf "mutator for %s changed no paths on disk" (MutationCaseId.value c.Id)
                                    )
                                )
                                ...
                                Error(
                                    MutationApplicationFailed(
                                        sprintf
                                            "claimed changed paths %A but actual paths were %A"
                                            (Set.toList claimedChanged)
                                            (Set.toList actualChanged)
                                    )
                                )
                            else
                                // Verify the receipt's own hashes against
                                // the executor's snapshots; rebuild the
                                // receipt from the independent observation.
                                let verifiedReceipt =
                                    { ChangedPaths = Set.toList actualChanged
                                      BeforeHashes =
                                        beforeSnap |> Map.filter (fun path _ -> Set.contains path actualChanged)
                                      AfterHashes =
                                        afterSnap |> Map.filter (fun path _ -> Set.contains path actualChanged) }

                                if not (verifiedReceipt.KeysMatch()) then
                                    Error(MutationApplicationFailed "verified receipt key sets are inconsistent")
                                else
                                    ...
                                    | Ok mutatedViolations ->
                                        ...
                                        let observedIds =
                                            mutatedViolations |> List.map (fun v -> v.Id) |> List.distinct |> Set.ofList
...
let executeMutationRegistry (cases: MutationCase list) : RegistryOutcome =
    ...
    | RegistryOk ->
        ...
        |> Map.ofList
        |> Ok
    | invalid -> Error(InvalidRegistry invalid)

/// Variant of ``executeMutationRegistry`` that accepts an explicit
/// workspace seam.  Tests use this to prove failure paths
/// deterministically.
let executeMutationRegistryWithSeam (cases: MutationCase list) (seam: WorkspaceSeam) : RegistryOutcome =
    match validateMutationRegistry cases with
    | RegistryOk -> cases |> List.map (fun c -> c.Id, executeCase c seam) |> Map.ofList |> Ok
    | invalid -> Error(InvalidRegistry invalid)
...
let executedCount (results: Map<MutationCaseId, Result<MutationSuccess, MutationFailure>>) : int = Map.count results
...
let renderFailureSummary (results: Map<MutationCaseId, Result<MutationSuccess, MutationFailure>>) : string =
    results
    |> Map.toList
    ...
    |> List.choose (fun (id, r) ->
        ...
        | Result.Error failure ->
            ...
            let detail =
                ...
                | UnexpectedViolation v -> sprintf "unexpected violation: %s %s %s" v.Id v.Path v.Detail
