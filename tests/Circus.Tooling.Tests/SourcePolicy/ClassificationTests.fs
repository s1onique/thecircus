...
let tests =
    testList
        "Approved and forbidden language classification"
        [ test ".fs is F# approved" {
          ...
          test ".elm is Elm approved" { Expect.isTrue (isApprovedElmExtension ".elm") "elm" }
          test ".sh is shell approved" { Expect.isTrue (isApprovedShellExtension ".sh") "sh" }
