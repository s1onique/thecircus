...
let private metadataEntries: CheckMetadataEntry list = CheckMetadataEntries

let private newTempDir () : string =
    let path =
        Path.Combine(Path.GetTempPath(), "circus-parity-" + Guid.NewGuid().ToString("n"))
...
let private writeStrictCsv (path: string) (rows: string list list) =
    ...
    let renderRow (cols: string list) =
        cols |> List.map quote |> String.concat ","
    ...
    testList
        ...
        [
          ...
          test "P1-1: valid concrete ID passes partition (validIds)" {
              ...
              let candidates =
                  [ ("CP-01_required_files", parseConcreteId "CP-01_required_files") ]
          ...
          test "P1-1: committed CSV validates with all defect collections empty" {
              ...
              | Failed(r, reasons) -> failtestf "parity validate failed: %s" (String.concat "; " reasons)
          }
          ...
          test "P1-1: CP-010_trusted_runner rejected (zero-padded prefix)" {
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      (List.contains "CP-010_trusted_runner" r.MalformedIdentities
                       || List.contains "CP-010_trusted_runner" r.UnknownIdentities)
                      "CP-010 must be flagged"
              | Ok _ -> failtestf "should have failed"
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      (List.contains "CP-10_trusted_runner-extra" r.MalformedIdentities
                       || List.contains "CP-10_trusted_runner-extra" r.UnknownIdentities)
                      "CP-10_trusted_runner-extra must be flagged"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: CP-10_trusted_runner description rejected (trailing text)" {
              ...
              let rows =
                  (metadataEntries
                   |> List.filter (fun m -> m.CheckId <> "CP-10_trusted_runner")
                   |> List.map (fun m -> [ m.CheckId; "desc"; m.CheckId; implLocFor m.CheckId; "p"; "n"; "complete" ]))
                  @ [ [ "CP-10_trusted_runner description"
                        "desc"
                        "CP-10_trusted_runner description"
                        "tools/Circus.Tooling/SourcePolicy/ContainerPolicy.fs (checkTrustedRunner)"
                        "p"
          ...
          test "P1-1: CP-10_trusted_runner/child rejected (path separator)" {
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      ...
                      "path separator must be malformed"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: cp-10_trusted_runner rejected (case variant)" {
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      ...
                      "case variant must be malformed"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: leading whitespace rejected" {
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      ...
                      "leading whitespace must be malformed"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: trailing whitespace rejected" {
              ...
              | Failed(r, _) ->
                  Expect.isTrue
                      (List.contains "CP-01_required_files " r.MalformedIdentities)
                      "trailing whitespace must be malformed"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: duplicate exact identity rejected" {
              ...
              match validateFile path with
              | Failed(r, _) -> Expect.contains r.DuplicateIdentities "CP-01_required_files" "duplicate must be flagged"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: exact unknown identity rejected (CP-99_unknown_check)" {
              ...
              match validateFile path with
              | Failed(r, _) -> Expect.contains r.UnknownIdentities "CP-99_unknown_check" "unknown must be flagged"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: duplicate production identity is diagnosed" {
              ...
              let actual = duplicateIds [ "CP-01_required_files"; "CP-01_required_files" ]
          ...
          test "P1-1: isolated unknown FsharpCheckId reported as unknown and as column mismatch" {
              ...
              | Failed(r, _) ->
                  ...
                  Expect.contains
                      r.FieldMismatches
                      ("CP-10_trusted_runner", "CP-99_extra_check", "legacy_check_id != fsharp_check_id")
                      "column mismatch must also be reported"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: metadataEntries.ImplementationFunctionName matches nameof for CP-01" {
              ...
              let cp01 =
                  List.find (fun (m: CheckMetadataEntry) -> m.CheckId = "CP-01_required_files") metadataEntries

              Expect.equal
                  cp01.ImplementationFunctionName
                  "checkRequiredFiles"
                  "CP-01 function name must be checkRequiredFiles"
          }

          test "P1-1: metadataEntries.ImplementationFunctionName matches nameof for CP-10" {
              let cp10 =
                  List.find (fun (m: CheckMetadataEntry) -> m.CheckId = "CP-10_trusted_runner") metadataEntries

              Expect.equal
                  cp10.ImplementationFunctionName
                  "checkTrustedRunner"
                  "CP-10 function name must be checkTrustedRunner"
          }
          ...
          test "P1-1: metadataEntries produces one exact map entry per CheckDefinition" {
              ...
              let metadataIds =
                  List.map (fun (m: CheckMetadataEntry) -> m.CheckId) metadataEntries
              ...
              Expect.equal
                  (List.length metadataIds)
                  (List.length metadataEntries)
                  "metadata count matches metadataEntries count"
          }
          ...
          test "P1-1: missing exact production identity fails validation" {
              ...
              match validateFile path with
              | Failed(r, _) -> Expect.contains r.MissingIdentities "CP-01_required_files" "missing must be flagged"
              | Ok _ -> failtestf "should have failed"
          ...
          test "P1-1: wrong implementation function for known identity rejected" {
                  ...
                  |> List.map (fun m ->
                      if m.CheckId = "CP-01_required_files" then
                          [ m.CheckId
                            "desc"
                            m.CheckId
                            "tools/Circus.Tooling/SourcePolicy/ContainerPolicy.fs (checkWrongFunction)"
                            "p"
                            "n"
                            "complete" ]
                      else
              ...
              | Failed(r, _) ->
                  Expect.isGreaterThan
                      (List.length r.IdentityPathFunctionMismatches)
                      0
                      "function mismatch must be flagged"
              | Ok _ -> failtestf "should have failed"
          ...
          test "invalid status rejected at parse" {
              ...
              let rows =
                  [ for m in metadataEntries ->
                        [ m.CheckId; "desc"; m.CheckId; implLocFor m.CheckId; "p"; "n"; "bogus" ] ]
