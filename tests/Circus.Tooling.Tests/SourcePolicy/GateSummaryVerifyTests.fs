...
let private writeTempJson (content: string) : string =
    let path =
        Path.Combine(Path.GetTempPath(), "circus-gate-summary-verify-" + System.Guid.NewGuid().ToString("n") + ".json")
...
let tests =
    testList
        "GateSummaryVerify structural validator"
        [ test "passing document validates with no failures" {
          ...
          test "non-canonical overall_status rejected" {
              let bad =
                  (passingDoc ()).Replace("\"overall_status\": \"pass\"", "\"overall_status\": \"green\"")
          ...
          test "invalid tested_tree_oid rejected" {
              let bad =
                  (passingDoc ()).Replace("\"0123456789abcdef0123456789abcdef01234567\"", "\"not-a-sha\"")
          ...
          test "count mismatch rejected" {
              ...
              | Error msg ->
                  ...
                  Expect.isTrue
                      (msg.Contains "count inconsistency" || msg.Contains "does not match")
