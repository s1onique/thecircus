...
let private writeAndLoad (content: string) =
    ...
    let tmp =
        System.IO.Path.Combine(System.IO.Path.GetTempPath(), "baseline-test-" + System.Guid.NewGuid().ToString("n"))

    System.IO.Directory.CreateDirectory tmp |> ignore

    System.IO.Directory.CreateDirectory(System.IO.Path.Combine(tmp, "factory"))
    |> ignore
