...
let private assertPair (label: string) (expectedStatus: string) (expectedExit: int) (actual: string * int) =
...
let classificationTests =
    testList
        "GateSummary classification seam"
        [ test "classifyOutcome: zero exit → (pass, 0)" {
              assertPair "Exited(0,_)" "pass" 0 (GateSummary.classifyOutcome (Exited(exitCode = 0, cleanupNote = "")))
          }
          test "classifyOutcome: non-zero exit → (fail, n)" {
              assertPair "Exited(7,_)" "fail" 7 (GateSummary.classifyOutcome (Exited(exitCode = 7, cleanupNote = "")))
          ...
          test "classifyOutcome: Cancelled → (unavailable, -1)" {
              assertPair "Cancelled" "unavailable" -1 (GateSummary.classifyOutcome (Cancelled(cleanupNote = "")))
          }
...
let identityTests =
    testList
        "GateSummary identity fail-closed"
        [ test "testedIdentityFromGit: missing commit identity → Error" {
              ...
              let tmp =
                  Path.Combine(Path.GetTempPath(), "circus-no-git-" + Guid.NewGuid().ToString("n"))
              ...
              try
                  ...
                  | Result.Ok identity -> failtestf "expected Error, got Ok %A" identity
              finally
                  try
    ...
    testList
        ...
        [ let emptyDoc: GateSummary.GateSummaryDoc =
          ...
          test "writeDocument: existing writable directory → Ok" {
              let tmp =
                  Path.Combine(Path.GetTempPath(), "circus-writer-" + Guid.NewGuid().ToString("n"))
              ...
              try
                  ...
                  | Ok() -> Expect.isTrue (File.Exists target) "artefact must be written"
          ...
          test "writeDocument: absent creatable directory → Ok and directory created" {
              let parent =
                  Path.Combine(Path.GetTempPath(), "circus-writer-parent-" + Guid.NewGuid().ToString("n"))
          ...
          test "writeDocument: existing canonical artefact not replaced by partial document" {
              ...
              let tmp =
                  Path.Combine(Path.GetTempPath(), "circus-writer-preserve-" + Guid.NewGuid().ToString("n"))
              ...
              try
                  ...
                  | Ok() -> failtestf "expected Error for path with empty parent"
...
let regenerateResultTests =
    testList
        "GateSummary regenerate result"
        [ test "regenerate: identity unreadable → Result.Error (IdentityReadFailed CommitOidMissing)" {
              let tmp =
                  Path.Combine(Path.GetTempPath(), "circus-no-git-regen-" + Guid.NewGuid().ToString("n"))
              ...
              try
                  ...
                  | Result.Error other -> failtestf "expected IdentityReadFailed CommitOidMissing, got %A" other
              finally
                  try
                      Directory.Delete(tmp, true)
                  with _ ->
...
let artefactNonMutationTests =
    testList
        "GateSummary artefact non-mutation"
        [ test "GateSummary ordinary tests never mutate .factory/gate-summary.json" {
              let canonical = Path.Combine(Path.GetFullPath ".", ".factory", "gate-summary.json")
              ...
              let tmp =
                  Path.Combine(Path.GetTempPath(), "circus-isolation-" + Guid.NewGuid().ToString("n") + ".json")
              ...
              | Some bytes ->
                  if File.Exists canonical then
                      ...
                      Expect.equal (afterBytes.Length = bytes.Length) true "canonical artefact length must be unchanged"
                      Expect.equal afterBytes bytes "canonical artefact must be byte-identical"
              | None -> Expect.isFalse (File.Exists canonical) "canonical artefact must not be created by tests"
          } ]
...
let wiringTests =
    testList
        ...
        [ test "wiring: source-policy-tests check is wired into the canonical gate" {
              ...
              let doc = GateSummary.buildDocument (Path.GetFullPath ".") identity runner
          ...
          test "wiring: source-policy-tests command is exactly make test-source-policy" {
              ...
              let inv =
                  ...
                  | Some i -> i
                  | None -> failtestf "expected source-policy-tests invocation but got: %A" (List.ofSeq invocations)
              ...
              Expect.equal sourcePolicyInvocations 1 "source-policy-tests must be invoked exactly once"
          }
          ...
          test "wiring: runner exit 0 maps source-policy-tests to pass" {
              ...
              Expect.equal doc.OverallStatus "pass" "overall status must be pass when source-policy-tests passes"
          }
          ...
          test "wiring: runner exit 1 maps source-policy-tests to fail" {
              ...
              Expect.equal doc.OverallStatus "fail" "overall status must be fail when source-policy-tests fails"
          }
          ...
          test "wiring: runner launch failure maps source-policy-tests to unavailable" {
              ...
              Expect.equal doc.OverallStatus "fail" "overall status must be fail when any check is unavailable"
          }
