...
let evidenceIdUniquenessTests =
    testList
        ...
        [ testCase "duplicate evidence IDs are detected"
          <| fun () ->
          ...
          testCase "evidence ID is deterministic from canonical form"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              ...
              for record in fixture.Records do
                  ...
                  Expect.isTrue
                      (System.Text.RegularExpressions.Regex.IsMatch(record.EvidenceId, "^[0-9a-f]{64}$"))
                      "evidence ID should be a valid SHA-256"

          testCase "evidence IDs are unique"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
...
let checkIdUniquenessTests =
    testList
        "CheckIdUniqueness"
        [ testCase "duplicate check IDs are rejected"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  if not (List.isEmpty fixture.Records) then
                      ...
                      let record2 =
                          { record1 with
                              EvidenceId = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" } // Different EvidenceId
          ...
          testCase "check IDs are sorted and unique in fixture"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
...
let idempotentPublicationTests =
    testList
        ...
        [ testCase "idempotent publication produces identical records content"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "idempotent publication produces identical aggregate SHA"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "idempotent publication produces identical compatibility projection"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  Expect.equal
                      content1
                      content2
                      "idempotent publication should produce identical compatibility projection"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "evidence ID derivation is injective"
          <| fun () ->
              let fixture = createValidPublicationFixture ()

              // Create a map of evidence ID to record
              let idToRecord =
                  fixture.Records |> List.map (fun r -> r.EvidenceId, r) |> Map.ofList
