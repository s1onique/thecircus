...
let private runCliBoundedWithTimeout
    (repoRoot: string)
    (args: string list)
    (timeout: TimeSpan)
...
let private createMinimalStructure (dir: string) : unit =
    ...
    let declarationsDir =
        Path.Combine(dir, canonical, "corpus", "episodes", "declarations")
        ...
        [
          ...
          testTask "inventory with missing evidence file => NonZeroExit" {
              ...
              try
                  ...
                  match result with
                  | Ok success -> failwithf "Expected NonZeroExit but got exit code %d" success.ExitCode
                  | Error(NonZeroExit _) ->
                      // Expected non-zero exit - evidence file missing
                      ()
                  | Error other -> failwithf "Expected NonZeroExit, got %A" other
              finally
                  cleanup dir
          }
          ...
          testTask "verify with malformed evidence => NonZeroExit" {
              ...
              try
                  ...
                  let evidencePath =
                      Path.Combine(dir, ".circus", "corpus", "normalized", "verification-evidence-v1.jsonl")
          ...
          testTask "help command succeeds" {
              ...
              try
                  ...
                  | Error failure -> failwithf "Help command failed: %A" failure
              finally
                  cleanup dir
          }
          ...
          testTask "verify with empty evidence file => exit 0" {
              ...
              try
                  ...
                  | Error(NonZeroExit _) ->
                      ...
                      ()
                  | Error other -> failwithf "Unexpected failure type: %A" other
              finally
                  cleanup dir
          }
          ...
          testTask "regenerate preserves canonical files on failure" {
              ...
              try
                  ...
                  let episodePath =
                      Path.Combine(dir, ".circus", "corpus", "normalized", "repair-episodes-v1.jsonl")
                  ...
                  // Read after
                  let contentAfter =
                      if File.Exists(episodePath) then
                          File.ReadAllText(episodePath)
                      else
                          ""
          ...
          testTask "BoundedProcess timeout mechanism works" {
              ...
              try
                  ...
                  let! result =
                      runCliBoundedWithTimeout dir [ "fsharp-diagnostics"; "repair-episodes"; "help" ] shortTimeout
