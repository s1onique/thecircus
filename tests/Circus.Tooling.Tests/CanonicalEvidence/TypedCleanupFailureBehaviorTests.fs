...
let idempotentWriteTests =
    testList "IdempotentWrites" [
        testCase "staging succeeds when compatibility file already exists" <| fun () ->
            ...
            try
                ...
                Expect.stringContains newContent "circus-canonical-evidence" "file should be overwritten with valid content"
            finally
                if Directory.Exists tempDir then Directory.Delete(tempDir, true)

        testCase "staging succeeds when aggregate file already exists" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
        ...
        testCase "staging succeeds when records file already exists" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
        ...
        testCase "repeated staging produces identical output" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let recordValidationFailureTests =
    testList "RecordValidationFailures" [
        testCase "validation fails with empty records list" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
                ...
                let outcome = stageAndPublishSnapshot tempDir [] fixture.Aggregate fixture.CompatibilityProjection None
        ...
        testCase "validation fails with duplicate record IDs" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
                ...
                if not (List.isEmpty fixture.Records) then
                    let duplicateRecord = { fixture.Records.Head with EvidenceId = fixture.Records.Head.EvidenceId }
...
let outputDirectoryCreationTests =
    testList "OutputDirectoryCreation" [
        testCase "staging creates nested output directory" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            ...
            try
                ...
                let fixture = createValidPublicationFixture ()
                let outcome = stageAndPublishSnapshot nestedDir fixture.Records fixture.Aggregate fixture.CompatibilityProjection None
        ...
        testCase "staging creates all required files" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let outcomeFieldPreservationTests =
    testList "OutcomeFieldPreservation" [
        testCase "outcome preserves all required fields" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
        ...
        testCase "failure preserves typed cleanup failure when present" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
        ...
        testCase "failure preserves typed failure with details" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let typedCleanupFailureBehaviorTests =
    testList "TypedCleanupFailureBehavior" [
