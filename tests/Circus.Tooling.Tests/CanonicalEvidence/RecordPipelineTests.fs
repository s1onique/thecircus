...
let validateBijectionTests =
    testList
        "validateBijection"
        [ testCase "empty definitions returns DefinitionsEmpty"
          <| fun () ->
          ...
          testCase "empty results returns ResultsEmpty"
          <| fun () ->
          ...
          testCase "duplicate definition ID returns DuplicateDefinitionId"
          <| fun () ->
              let defs = [ sampleDefinition "check1" true; sampleDefinition "check1" true ]

              let res =
                  [ sampleResult "check1" [ "dotnet"; "build" ] Pass
                    sampleResult "check2" [ "make" ] Pass ]
          ...
          testCase "duplicate result ID returns DuplicateResultId"
          <| fun () ->
              let defs = [ sampleDefinition "check1" true; sampleDefinition "check2" true ]

              let res =
                  [ sampleResult "check1" [ "dotnet" ] Pass
                    sampleResult "check1" [ "dotnet" ] Pass ]
          ...
          testCase "missing result returns DefinitionMissingResult"
          <| fun () ->
          ...
          testCase "missing definition returns ResultMissingDefinition"
          <| fun () ->
              let defs = [ sampleDefinition "check1" true ]
          ...
          testCase "matching pairs returns Ok"
          <| fun () ->
              let defs = [ sampleDefinition "check1" true; sampleDefinition "check2" true ]
...
let validateRecordsTests =
    testList
        "validateRecords"
        [ testCase "empty records returns RecordsEmpty issue"
          <| fun () ->
          ...
          testCase "valid records returns Valid"
          <| fun () ->
...
let failureToStringTests =
    testList
        "recordPipelineFailureToString"
        [ testCase "DefinitionsEmpty has correct message"
          <| fun () ->
          ...
          testCase "ResultsEmpty has correct message"
          <| fun () ->
          ...
          testCase "DuplicateDefinitionId includes ID"
          <| fun () ->
              let msg =
                  recordPipelineFailureToString (RecordPipelineFailure.DuplicateDefinitionId "check1")

              Expect.stringContains msg "check1" "expected check ID in message"

          testCase "EmptyCommand includes ID"
          <| fun () ->
...
let validationIssueToStringTests =
    testList
        "recordValidationIssueToString"
        [ testCase "RecordsEmpty has correct message"
          <| fun () ->
          ...
          testCase "EvidenceIdEmpty includes ID"
          <| fun () ->
              let msg =
                  recordValidationIssueToString (RecordValidationIssue.EvidenceIdEmpty "check1")

              Expect.stringContains msg "check1" "expected check ID"
              Expect.stringContains msg "empty" "expected empty message"

          testCase "SubjectMismatch includes all details"
          <| fun () ->
              let msg =
                  recordValidationIssueToString (RecordValidationIssue.SubjectMismatch("check1", "commit1", "commit2"))
...
let executedCanonicalCheckTests =
    testList
        ...
        [ testCase "can create with definition result and startedAt"
          <| fun () ->
          ...
          testCase "multiple executed checks have different timestamps"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
...
let convertExecutedChecksToRecordsTests =
    testList
        "convertExecutedChecksToRecords"
        [ testCase "one executed check produces one record"
          <| fun () ->
              let def = sampleDefinition "check1" true

              let res =
                  { (sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass) with
                      FailureKind = Some "test_failure" }
          ...
          testCase "ten executed checks produce ten records"
          <| fun () ->
              let executedChecks =
                  [ for i in 1..10 do
                        ...
                        let res =
                            sampleResult (sprintf "check%d" i) [ "dotnet"; "build" ] EvidenceStatus.Pass
              ...
              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.equal (List.length records) 10 "expected 10 records"

          testCase "record ID is nonempty"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
              ...
              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.isNonEmpty records.Head.EvidenceId "expected nonempty EvidenceId"

          testCase "record ID is 64 lowercase hexadecimal characters"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
              ...
              | Ok records ->
                  ...
                  Expect.isTrue
                      (System.Text.RegularExpressions.Regex.IsMatch(id, "^[0-9a-f]{64}$"))
                      "expected lowercase hex"

          testCase "record ID recomputes"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
          ...
          testCase "subject commit is preserved"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
              ...
              let result =
                  convertExecutedChecksToRecords [ executed ] "abc123def456" "tree789" true

              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.equal records.Head.TestedCommitOid "abc123def456" "expected commit preserved"

          testCase "subject tree is preserved"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
              ...
              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.equal records.Head.TestedTreeOid "tree789xyz" "expected tree preserved"

          testCase "working tree clean is preserved"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
              ...
              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.isFalse records.Head.WorkingTreeClean "expected dirty tree"

          testCase "FailureKind is preserved"
          <| fun () ->
              let def = sampleDefinition "check1" true

              let res =
                  { (sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Fail) with
                      FailureKind = Some "non_zero_exit:42" }
              ...
              match result with
              | Error e -> failwithf "expected Ok, got Error: %A" e
              | Ok records -> Expect.equal records.Head.FailureKind (Some "non_zero_exit:42") "expected FailureKind"

          testCase "valid records pass validation"
          <| fun () ->
              let def = sampleDefinition "check1" true
              let res = sampleResult "check1" [ "dotnet"; "build" ] EvidenceStatus.Pass
              let startedAt = DateTimeOffset(2024, 1, 1, 0, 0, 0, TimeSpan.Zero)
...
let aggregateDerivationTests =
    testList
        "AggregateDerivation"
        [ testCase "required pass -> required_failed=0, overall pass"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check1") with
                        Required = true
                        Result = RecordPass } ]
          ...
          testCase "required fail -> required_failed=1, overall fail"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check1") with
                        Required = true
          ...
          testCase "required unavailable -> required_failed=1, overall fail"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check1") with
                        Required = true
                        Result = RecordUnavailable } ]
          ...
          testCase "optional unavailable -> required_failed unchanged"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check1") with
                        Required = true
                        Result = RecordPass }
                    { (sampleRecord "check2") with
                        Required = false
                        Result = RecordUnavailable } ]
          ...
          testCase "record IDs are sorted"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check3") with
                        EvidenceId = "00003" }
                    { (sampleRecord "check1") with
                        EvidenceId = "00001" }
                    { (sampleRecord "check2") with
          ...
          testCase "aggregate semantic hash recomputes"
          <| fun () ->
              let records =
                  [ { (sampleRecord "check1") with
                        EvidenceId = "a".PadRight(64, '0') } ]
...
let executedCheckStartTimeTests =
    testList
        "ExecutedCheckStartTime"
        [ testCase "each check gets its own start time"
          <| fun () ->
              ...
              let executed1 =
                  { ExecutedCanonicalCheck.Definition = def1
                    Result = res1
                    StartedAt = time1 }

              let executed2 =
                  { ExecutedCanonicalCheck.Definition = def2
                    Result = res2
                    StartedAt = time2 }
                  ...
                  Expect.isTrue
                      (errors
                       |> List.exists (function
                           | EvidenceWireParseError.InvalidTimestamp _ -> true
                           | _ -> false))
                      "Expected InvalidTimestamp error"

          testCase "manifest valid with exactly three required paths"
          <| fun () ->
              let manifest =
                  "{\"path\":\"records.jsonl\",\"sha256\":\"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\",\"byte_length\":100}\n{\"path\":\"aggregate.json\",\"sha256\":\"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\",\"byte_length\":200}\n{\"path\":\"canonical-evidence.json\",\"sha256\":\"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\",\"byte_length\":300}\n"

              match parseArtifactManifestJsonlStrict manifest with
              | Ok entries -> Expect.equal (List.length entries) 3 "Expected exactly 3 entries"
              | Error e -> failwithf "Expected valid manifest: %A" e

          testCase "manifest missing records.jsonl rejects"
          <| fun () ->
              let manifest =
                  "{\"path\":\"aggregate.json\",\"sha256\":\"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\",\"byte_length\":200}\n{\"path\":\"canonical-evidence.json\",\"sha256\":\"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855\",\"byte_length\":300}\n"
              ...
              | Error errors ->
                  Expect.isTrue
                      (errors
                       |> List.exists (function
                           | ArtifactManifestParseError.MissingRequiredPath "records.jsonl" -> true
                           | _ -> false))
                      "Expected MissingRequiredPath error"

          testCase "manifest unknown path rejects"
          <| fun () ->
              let manifest =
              ...
              | Error errors ->
                  Expect.isTrue
                      (errors
                       |> List.exists (function
                           | ArtifactManifestParseError.UnknownPath "extra-file.json" -> true
                           | _ -> false))
                      "Expected UnknownPath error" ]
