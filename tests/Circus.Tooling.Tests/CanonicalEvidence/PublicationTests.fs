...
let publicationFixtureTests =
    testList "PublicationFixture" [
        testCase "createValidPublicationFixture produces valid fixture" <| fun () ->
        ...
        testCase "fixture compatibility has recomputable SemanticSha256" <| fun () ->
            ...
            Expect.equal recomputed fixture.CompatibilityProjection.SemanticSha256 "compatibility SemanticSha256 should recompute"
    ]
...
let stagedSnapshotRoundTripTests =
    testList "StagedSnapshotRoundTrip" [
        testCase "stageAndPublishSnapshot writes and validates all four files" <| fun () ->
...
let stagedCorruptionTests =
    testList "StagedCorruption" [
        testCase "record corruption is rejected" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let aggregateRecomputationTests =
    testList "AggregateRecomputation" [
        testCase "aggregate recomputed from parsed records matches stored" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let artifactManifestAuthorityTests =
    testList "ArtifactManifestAuthority" [
        testCase "manifest has exactly three required paths" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
...
let compatibilityEquivalenceTests =
    testList "CompatibilityEquivalence" [
        testCase "compatibility projection matches provider output" <| fun () ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
            Directory.CreateDirectory tempDir |> ignore
            try
                let fixture = createValidPublicationFixture ()
                ...
                | Ok parsed ->
                    Expect.equal parsed.TestedCommitOid fixture.CompatibilityProjection.TestedCommitOid "commit should match"
                    Expect.equal parsed.TestedTreeOid fixture.CompatibilityProjection.TestedTreeOid "tree should match"
                    Expect.equal parsed.SemanticSha256 fixture.CompatibilityProjection.SemanticSha256 "semantic hash should match"
