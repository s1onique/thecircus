    ...
    |> List.exists (function
        | StagedSnapshotFailure.AggregateFieldMismatch diff ->
            ...
            | AggregateDifference.SubjectCommitOid(e0, a0), AggregateDifference.SubjectCommitOid(e1, a1) ->
                e0 = e1 && a0 = a1
            | AggregateDifference.SubjectTreeOid(e0, a0), AggregateDifference.SubjectTreeOid(e1, a1) ->
                e0 = e1 && a0 = a1
            ...
            | AggregateDifference.SemanticSha256(e0, a0), AggregateDifference.SemanticSha256(e1, a1) ->
                e0 = e1 && a0 = a1
            | _ -> false
        | _ -> false)

/// Check if failures contain AggregateSemanticHashMismatch with expected values
let private containsSemanticHashMismatch
    (failures: StagedSnapshotFailure list)
    (expectedHash: string)
    (corruptedHash: string)
    : bool =
    failures
    |> List.exists (function
        | StagedSnapshotFailure.AggregateSemanticHashMismatch(exp, act) -> exp = expectedHash && act = corruptedHash
        | _ -> false)
...
let private verifyFilesPreserved
    (original: Map<string, byte array option>)
    (current: Map<string, byte array option>)
    : bool =
    ...
    if originalKeys <> currentKeys then
        false
    else
        // Now verify each file's bytes are exactly the same
        original
        |> Map.forall (fun filename origBytes ->
...
let fourFilePreservationTests =
    testList
        ...
        [ testCase "aggregate mutation preserves all four live files byte-identically"
          <| fun () ->
              ...
              try
                      ...
                      try
                          ...
                          let artifactsLine =
                              sprintf
                                  ...
                                  (Circus.Tooling.FSharpDiagnostics.Hashing.sha256Hex recordsBytes)
                                  recordsBytes.Length

                          let aggArtifactLine =
                              sprintf
                                  """{"path":"aggregate.json","sha256":"%s","byte_length":%d}"""
                                  (Circus.Tooling.FSharpDiagnostics.Hashing.sha256Hex aggBytes)
                                  aggBytes.Length

                          let compatArtifactLine =
                              sprintf
                                  """{"path":"canonical-evidence.json","sha256":"%s","byte_length":%d}"""
                                  (Circus.Tooling.FSharpDiagnostics.Hashing.sha256Hex compatBytes)
                                  compatBytes.Length
                  ...
                  Expect.isTrue
                      (verifyFilesPreserved liveSnapshotBefore liveSnapshotAfter)
                      "all four live files should be byte-identical after rejected publication"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "semantic hash corruption preserves all four live files byte-identically"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  let corruptedAggregate =
                      { fixture.Aggregate with
                          SemanticSha256 = corruptedHash }
...
let derivedFieldMutationTests =
    testList
        "DerivedFieldMutation"
        [ testCase "records_total mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "records_passed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RecordsPassed _) ->
                                  true
                              | _ -> false)
          ...
          testCase "records_failed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RecordsFailed _) ->
                                  true
                              | _ -> false)
          ...
          testCase "records_unavailable mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RecordsUnavailable _) ->
                                  true
                              | _ -> false)
          ...
          testCase "tests_total mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "tests_passed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "tests_failed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "tests_ignored mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "tests_errored mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "required_checks_total mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RequiredChecksTotal _) ->
                                  true
                              | _ -> false)
          ...
          testCase "required_checks_passed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RequiredChecksPassed _) ->
                                  true
                              | _ -> false)
          ...
          testCase "required_checks_failed mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.RequiredChecksFailed _) ->
                                  true
                              | _ -> false)
          ...
          testCase "record_ids mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "overall_status mutation is detected via field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let hasFieldMismatch =
                          failures
                          |> List.exists (function
                              | StagedSnapshotFailure.AggregateFieldMismatch(AggregateDifference.OverallStatus _) ->
                                  true
                              | _ -> false)
...
let stagedRecordDivergenceTests =
    testList
        "StagedRecordDivergence"
        [ testCase "mutated records.jsonl without aggregate update is rejected"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
          ...
          testCase "mutated status in records without aggregate update is rejected"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      ...
                      Expect.isTrue
                          (hasRecordsPassedMismatch || hasRecordsFailedMismatch || hasOverallStatusMismatch)
...
let malformedRecordIsolationTests =
    testList
        "MalformedRecordIsolation"
        [ testCase "malformed records.jsonl triggers parse failure, not aggregate authority"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      ...
                      Expect.isTrue
                          (noAggregateFieldMismatch failures)
                          (sprintf
                              "Should NOT have any AggregateFieldMismatch for malformed records. Failures: %A"
                              failures)
                      ...
                      Expect.isTrue
                          (noRecordValidationFailure failures)
          ...
          testCase "records.jsonl with invalid line triggers parse failure"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
...
let semanticHashSelfIntegrityTests =
    testList
        "SemanticHashSelfIntegrity"
        [ testCase "corrupted semantic_sha256 triggers self-integrity failure, not field mismatch"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      ...
                      Expect.isTrue
                          (containsSemanticHashMismatch failures fixture.Aggregate.SemanticSha256 corruptedHash)
                      ...
                      Expect.isTrue
                          (noSemanticSha256FieldMismatch failures)
...
let subjectOidMutationTests =
    testList
        "SubjectOidMutation"
        [ testCase "subject_commit_oid mutation is rejected"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  let mutatedAggregate =
                      { fixture.Aggregate with
                          SubjectCommitOid = testCommitOid2 }
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let expectedDiff =
                          AggregateDifference.SubjectCommitOid(fixture.Aggregate.SubjectCommitOid, testCommitOid2)

                      Expect.isTrue
                          (containsStagedFieldMismatch failures expectedDiff)
          ...
          testCase "subject_tree_oid mutation is rejected"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some(PublicationFailure.SnapshotStagedValidationFailed failures) ->
                      let expectedDiff =
                          AggregateDifference.SubjectTreeOid(fixture.Aggregate.SubjectTreeOid, testTreeOid2)
...
let schemaVersionMutationTests =
    testList
        "SchemaVersionMutation"
        [ testCase "schema_version mutation is rejected with parse failure"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              ...
              try
                  ...
                  | Some other -> failwithf "Expected SnapshotStagedValidationFailed, got %A" other
                  | None -> failwith "Expected failure but got Success"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true) ]
...
let baselinePublicationTests =
    testList
        "BaselinePublication"
        [ testCase "valid snapshot publishes successfully"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
