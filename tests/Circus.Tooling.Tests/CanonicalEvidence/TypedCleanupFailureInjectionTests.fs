...
let private publishAndCaptureSnapshot
    ...
    Expect.isTrue (publicationSucceeded outcome) (sprintf "initial snapshot publication failed: %A" outcome)
...
let private assertCleanupFailure
    ...
    Expect.hasLength cleanupPaths 1 "cleanup must be called exactly once"
    ...
    Expect.isTrue
        (Path.GetFileName(observedPath).StartsWith(".staging.", StringComparison.Ordinal))
        "cleanup must target the generated staging directory"

    match outcome.CleanupFailure with
    | Some failure ->
        Expect.equal failure.Operation DeleteStagingDirectory "cleanup operation must be exact"

        Expect.equal failure.Path observedPath "cleanup failure must use the actual staging path"

        Expect.equal failure.Detail expectedDetail "cleanup detail must be preserved"

    | None -> failtest "expected cleanup failure"
...
let testA_successfulPublicationAndSuccessfulCleanup =
    testCase "TestA: successful publication and successful cleanup"
    <| fun () ->
        ...
        try
            ...
            Expect.isNone outcome.Failure "primary publication should succeed (no failure)"

            // Prove: cleanup succeeded
            Expect.isNone outcome.CleanupFailure "cleanup should succeed (no cleanup failure)"

            // Prove: cleanup dependency called exactly once
            Expect.hasLength cleanupPaths 1 "cleanup dependency must be invoked exactly once"
            ...
            Expect.isEmpty stagingDirs "no staging directory should remain after successful cleanup"

            // Prove: all four live files exist
            for filename in snapshotFiles do
                let path = Path.Combine(tempDir, filename)
                Expect.isTrue (File.Exists path) (sprintf "live file %s should exist" filename)
...
let testB_successfulPublicationAndCleanupFailure =
    testCase "TestB: successful publication and cleanup failure"
    <| fun () ->
        let fixture = createValidPublicationFixture ()
        let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
        Directory.CreateDirectory(tempDir) |> ignore
        ...
        try
            ...
            Expect.isNone outcome.Failure "primary publication should succeed even when cleanup fails"
            ...
            | Some cf ->
                ...
                Expect.notEqual cf.Path untrustedPath "dependency-controlled paths must not escape the cleanup boundary"

                Expect.equal cf.Path cleanupPaths.[0] "cleanup failure must identify the actual staging path"

                // Prove: operation is DeleteStagingDirectory
                Expect.equal cf.Operation DeleteStagingDirectory "cleanup operation should be DeleteStagingDirectory"

                // Prove: detail is preserved
                Expect.equal cf.Detail "injected cleanup failure" "cleanup failure detail should be preserved"
            ...
            Expect.equal outcome.StagingState StagingMayRemain "staging directory may remain after cleanup failure"
            ...
            Expect.isFalse (publicationSucceeded outcome) "publicationSucceeded should be false when cleanup fails"
        finally
            if Directory.Exists tempDir then
                Directory.Delete(tempDir, true)
...
let testC_validationRejectionAndCleanupFailure =
    testCase "TestC: validation rejection and cleanup failure"
    <| fun () ->
        let fixture = createValidPublicationFixture ()
        let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
        Directory.CreateDirectory(tempDir) |> ignore
        ...
        try
            ...
            Expect.equal
                outcome.ReplacementPhaseInvocationCount
                0
            ...
            Expect.isTrue
                (verifyFilesPreserved previousSnapshot currentSnapshot)
...
let testD_mutationHookRejectionAndCleanupFailure =
    testCase "TestD: mutation-hook rejection and cleanup failure"
    <| fun () ->
        let fixture = createValidPublicationFixture ()
        let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
        Directory.CreateDirectory(tempDir) |> ignore
        ...
        try
            ...
            let mutationHook (stagingDir: string) : Result<unit, string> = Error "injected mutation rejection"
            ...
            | Some(SnapshotMutationHookFailed detail) ->
                Expect.equal detail "injected mutation rejection" "mutation rejection detail must be exact"
            | other -> failtestf "expected SnapshotMutationHookFailed, got %A" other
...
let testE_cleanupDependencyThrows =
    testCase "TestE: cleanup dependency throws"
    <| fun () ->
        let fixture = createValidPublicationFixture ()
        let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
        Directory.CreateDirectory(tempDir) |> ignore
        ...
        try
            ...
            | Some cf ->
                ...
                let observedPath = cleanupPaths.[0]
                Expect.equal cf.Path observedPath "cleanup failure path should be the actual staging path"

                // Prove: detail contains the injected exception type and message
                Expect.stringContains cf.Detail "IOException" "cleanup failure detail should contain exception type"
...
let testF_validationRejectionAndSuccessfulCleanup =
    testCase "TestF: validation rejection and successful cleanup"
    <| fun () ->
        let fixture = createValidPublicationFixture ()
        let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
        Directory.CreateDirectory(tempDir) |> ignore
        ...
        try
            ...
            Expect.isNone outcome.CleanupFailure "cleanup should succeed"
            ...
            Expect.isEmpty stagingDirs "no staging directory should remain"
...
let invariantTests =
    testList
        "Invariants"
        [ testCase "CleanupSucceeded implies StagingRemoved"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  Expect.isNone outcome.CleanupFailure "fixture must produce CleanupSucceeded"

                  Expect.equal outcome.StagingState StagingRemoved "CleanupSucceeded must imply StagingRemoved"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "CleanupFailed implies StagingMayRemain"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  Expect.isSome outcome.CleanupFailure "fixture must produce CleanupFailed"

                  Expect.equal outcome.StagingState StagingMayRemain "CleanupFailed must imply StagingMayRemain"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "Published and CleanupSucceeded implies publicationSucceeded = true"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  Expect.isNone outcome.Failure "fixture must produce a published primary outcome"

                  Expect.isNone outcome.CleanupFailure "fixture must produce successful cleanup"

                  Expect.isTrue (publicationSucceeded outcome) "Published plus CleanupSucceeded must be successful"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "Published and CleanupFailed implies publicationSucceeded = false"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  Expect.isNone outcome.Failure "fixture must produce Published (primary did not fail)"
          ...
          testCase "Pre-replacement rejection implies LiveSnapshotUnchanged"
          <| fun () ->
              let fixture = createValidPublicationFixture ()
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  let mutationHook (_: string) : Result<unit, string> = Error "rejection"
                  ...
                  Expect.isSome outcome.Failure "fixture must produce a rejection (pre-replacement failure)"
