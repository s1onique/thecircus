        ...
        [ testCase "previous snapshot preserved on validation failure"
          <| fun () ->
              ...
              try
                  ...
                  Expect.isTrue
                      outcome2.PreviousSnapshotPreserved
                      "previous snapshot should be preserved on validation failure"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "previous snapshot preserved on staging error"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "first staging has no previous to preserve"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  Expect.isTrue
                      outcome.PreviousSnapshotPreserved
                      "previous snapshot preserved flag should be true (no previous to overwrite)"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true) ]
...
let liveSnapshotMayHaveChangedTests =
    testList
        ...
        [ testCase "live snapshot not changed on successful staging"
          ...
          testCase "live snapshot may have changed on failure"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  Expect.isFalse
                      (liveSnapshotMayHaveChanged outcome2.LiveSnapshotState)
                      "live snapshot may have changed should be false (restored)"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true) ]
...
let noPartialWritesTests =
    testList
        "NoPartialWrites"
        [ testCase "no partial files on successful staging"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "no partial files on staging failure"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "idempotent staging produces consistent files"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  let secondCompat =
                      File.ReadAllText(Path.Combine(tempDir, "canonical-evidence.json"))
                  ...
                  Expect.equal
                      secondCompat
                      firstCompat
                      "canonical-evidence.json should be identical on idempotent write"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true) ]
...
let snapshotContentValidationTests =
    testList
        ...
        [ testCase "published snapshot has correct record count"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
          ...
          testCase "published snapshot has correct aggregate SHA"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
              ...
              try
                  ...
                  Expect.equal
                      outcome.AggregateSha256
                      fixture.Aggregate.SemanticSha256
                      "aggregate SHA should match fixture"
              finally
                  if Directory.Exists tempDir then
                      Directory.Delete(tempDir, true)

          testCase "published snapshot path is set correctly"
          <| fun () ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("n"))
              Directory.CreateDirectory tempDir |> ignore
...
let partialReplacementAndRestorationTests =
    testList
        "PartialReplacementAndRestoration"
