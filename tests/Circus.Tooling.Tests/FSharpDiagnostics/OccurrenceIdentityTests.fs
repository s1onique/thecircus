...
let baseOcc =
    mkOcc Warning "FS0001" "hello" "src/Foo.fs"
        { StartLine = Some 10
          StartColumn = Some 5
          EndLine = Some 10
          EndColumn = Some 10 }
...
[<Tests>]
let tests =
    testList "FSharpDiagnostics.OccurrenceIdentity" [
      ...
      test "different start_column changes fingerprint" {
        let other = { baseOcc with Span = { baseOcc.Span with StartColumn = Some 6 } }
      ...
      test "different end_line changes fingerprint" {
        let other = { baseOcc with Span = { baseOcc.Span with EndLine = Some 11 } }
      ...
      test "different end_column changes fingerprint" {
        let other = { baseOcc with Span = { baseOcc.Span with EndColumn = Some 11 } }
      ...
      test "different message_normalized changes fingerprint" {
        let other = { baseOcc with MessageNormalized = "different" }
      ...
      test "different capture_id does NOT change fingerprint" {
        let other = { baseOcc with CaptureId = "other-capture" }
      ...
      test "two messages at same coordinate are different fingerprints" {
        let a =
            mkOcc Warning "FS0001" "first message" "src/Foo.fs"
                { StartLine = Some 10
                  StartColumn = Some 5
                  EndLine = Some 10
                  EndColumn = Some 10 }
        let b =
            mkOcc Warning "FS0001" "second message" "src/Foo.fs"
                { StartLine = Some 10
                  StartColumn = Some 5
                  EndLine = Some 10
                  EndColumn = Some 10 }
