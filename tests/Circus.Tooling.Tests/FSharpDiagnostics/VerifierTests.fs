...
let private makeRoot () =
    let root = Path.Combine(Path.GetTempPath(), "fd-verifier-" + System.Guid.NewGuid().ToString("N"))
...
let private writeCapture (repoRoot: string) (captureId: string) (rawContent: string) =
    ...
    let manifest = {
        ...
        SourceRootAliases = [ { AbsoluteRoot = "/home/me/project"; CanonicalRoot = "<REPO>" } ]
...
[<Tests>]
let tests =
    testList "FSharpDiagnostics.Verifier" [
      test "end-to-end pipeline runs on synthetic capture" {
        let root = makeRoot()
        try
            writeCapture root "cap-1" "/home/me/project/src/Foo.fs(1,1): warning FS0001: x\n/home/me/project/src/Foo.fs(1,1): error FS0002: y"
