...
let private makeRoot () =
    let root =
        Path.Combine(Path.GetTempPath(), "fd-inv-" + System.Guid.NewGuid().ToString("N"))
    ...
    testList
        ...
        [ test "authorityFor canonical root returns canonical_corpus" {
          ...
          test "classifyCanonicalPath classifies correctly" {
              Expect.equal
                  (classifyCanonicalPath "factory/evidence/fsharp-diagnostics/schemas/x.schema.json")
                  Derived
                  "schema is derived"

              Expect.equal
                  (classifyCanonicalPath "factory/evidence/fsharp-diagnostics/corpus/raw/cap-1/x.binlog")
                  Raw
                  "raw"

              Expect.equal
                  (classifyCanonicalPath "factory/evidence/fsharp-diagnostics/corpus/normalized/occurrences-v1.jsonl")
                  Normalized
                  "normalized"

              Expect.equal
                  (classifyCanonicalPath "factory/evidence/fsharp-diagnostics/corpus/manifests/artifacts-v1.jsonl")
                  Normalized
                  "manifests normalized"

              Expect.equal
                  (classifyCanonicalPath "factory/evidence/fsharp-diagnostics/fixtures/fsb-0022/x.log")
                  SourceSnapshot
                  "fixture snapshot"
          }
