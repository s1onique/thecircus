...
let testValidMinimalEnvelope () =
    ...
    match result with
    | Ok validated ->
        Expect.equal (EventId.value validated.EventId) "019b0437-2766-7a20-9225-4ab1645ba115" "EventId"
    | Error errs ->
...
let testMalformedJson () =
    ...
    match List.tryPick (function MalformedJson m -> Some m | _ -> None) violations with
    | Some msg ->
...
let testOversizedBodyRejected () =
    let baseline = System.Text.Encoding.UTF8.GetBytes(Fixtures.readFixture "valid/started-minimal.json")
...
let testRootArrayRejected () =
    ...
    Expect.isTrue
        (Assertions.hasRootMustBeObject (Assertions.contractViolations result))
        "RootMustBeObject present"
