...
let testRepositoryRefRequired () =
    let result = EventDecoder.decode maxBytes (Fixtures.bytes "invalid-started/started-missing-repository.json")
    Expect.isTrue (Assertions.hasInvalidKnownPayload (Assertions.contractViolations result)) "must reject as InvalidKnownPayload"
    Expect.isTrue
...
let testOversizedStringsRejected () =
    let result = EventDecoder.decode maxBytes (Fixtures.bytes "invalid-started/started-invalid-leamas-version.json")
    Expect.isTrue (Assertions.hasInvalidKnownPayload (Assertions.contractViolations result)) "InvalidKnownPayload present"
