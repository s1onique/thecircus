...
let createTestEntry (position: int64) (source: string) (eventId: string) (instanceId: string) (epochId: Guid) (sequence: int64) (runId: Guid) (envelope: string) =
    ...
    testList
        ...
        [ test "classifyCollision: error when no conflicts (should not happen)" {
          ...
          test "classifyCollision: identity conflict when identity exists but sequence doesn't" {
              let existing = createTestEntry 42L "source1" "id1" "inst1" (Guid.NewGuid()) 1L (Guid.NewGuid()) "{}"
          ...
          test "classifyCollision: sequence conflict when identity doesn't exist" {
              let existing = createTestEntry 42L "source2" "id2" "inst1" (Guid.NewGuid()) 1L (Guid.NewGuid()) "{}"
          ...
          test "classifyCollision: cross-identity conflict when different rows" {
              let byIdentity = createTestEntry 42L "source1" "id1" "inst1" (Guid.NewGuid()) 1L (Guid.NewGuid()) "{}"
              let bySequence = createTestEntry 99L "source2" "id2" "inst1" (Guid.NewGuid()) 1L (Guid.NewGuid()) "{}"

              let result = JournalDecision.classifyCollision (Some byIdentity) (Some bySequence) true
          ...
          test "isKnownEventType: started is known" {
              Expect.isTrue (JournalDecision.isKnownEventType "io.leamas.execution.started.v1")
          }

          test "isKnownEventType: finished is known" {
              Expect.isTrue (JournalDecision.isKnownEventType "io.leamas.execution.finished.v1")
          }

          test "isKnownEventType: unknown event type" {
              Expect.isFalse (JournalDecision.isKnownEventType "io.leamas.execution.artefact.published.v3")
          } ]
