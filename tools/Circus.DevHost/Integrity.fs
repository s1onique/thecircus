...
let verifyFile (path: string) (expected: string) (algorithm: IntegrityAlgorithm) : Result<unit, DevHostFailure> =
    let actual =
    ...
    | Error f -> Error f
    | Ok h ->
        if constantTimeEqualHex (h.ToLowerInvariant()) (expected.ToLowerInvariant()) then
            Ok()
        else
            Error(IntegrityFailure(path, expected, h))
