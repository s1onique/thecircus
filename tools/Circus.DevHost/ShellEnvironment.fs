...
let shellEscape (value: string) : string = "'" + value.Replace("'", "'\\''") + "'"
...
let renderPath (entries: string list) : string = dedupPath entries |> String.concat ":"
...
let renderEnvironment (spec: EnvironmentSpec) : string =
    ...
    let all = exports @ [ "PATH", renderPath combinedPath ]
...
let specFromLayout (layout: Paths.Layout) (existingPath: string) (nodeVersion: string) : EnvironmentSpec =
