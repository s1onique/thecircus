...
let verifyNode
    ...
    if not (File.Exists nodeBin) then Error(MissingTool Node)
    else
        let spec =
        ...
        | Ok r ->
            ...
            else Error(WrongToolVersion(Node, expected, actual))
...
let installNode
    ...
    async {
        ...
        let tempShasums = Path.Combine(Path.GetTempDirectory(), "node-shasums-" + Guid.NewGuid().ToString("n") + ".txt")
        let tempArchive = Path.Combine(Path.GetTempDirectory(), archiveName)

        let! shOutcome = http.Download(shasumsUrl, tempShasums, "", cancellation)
        match shOutcome with
        | Error e ->
            (try if File.Exists tempShasums then File.Delete tempShasums with _ -> ())
            return Error e
        | Ok _ ->
            let shasumsText =
                try File.ReadAllText tempShasums
                with ex ->
                    (try if File.Exists tempShasums then File.Delete tempShasums with _ -> ())
                    "err"
            (try if File.Exists tempShasums then File.Delete tempShasums with _ -> ())
            if shasumsText = "err" then
                return Error(DownloadFailure(shasumsUrl, "read failed"))
            else
                match parseShasums shasumsText archiveName with
                | Error e -> return Error e
                | Ok expectedHash ->
                    let! dlOutcome = http.Download(archiveUrl, tempArchive, expectedHash, cancellation)
                    match dlOutcome with
                    | Error e ->
                        (try if File.Exists tempArchive then File.Delete tempArchive with _ -> ())
                        return Error e
                    | Ok _ ->
                        match cachePlace cacheDir archiveName expectedHash tempArchive with
                        | Error e ->
                            (try if File.Exists tempArchive then File.Delete tempArchive with _ -> ())
                            return Error e
                        | Ok cached ->
                            let extractResult = safeExtract runner cached destDir
                            (try if File.Exists tempArchive then File.Delete tempArchive with _ -> ())
