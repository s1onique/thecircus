...
let readDotNetVersion (repoRoot: string) : Result<ToolVersion, DevHostFailure> =
    ...
    if not (File.Exists path) then Error(MissingAuthorityFile path)
    else
        try
            let text = File.ReadAllText path
            let m = Regex.Match(text, "\"version\"\s*:\s*\"([0-9]+\.[0-9]+\.[0-9]+)\"")
            if not m.Success then Error(MalformedAuthorityFile path)
            else
                let raw = m.Groups.[1].Value
                match ToolVersion.parse raw with
                | Ok v -> Ok v
                | Error _ -> Error(MalformedAuthorityFile path)
        with _ -> Error(MalformedAuthorityFile path)
...
let readNodeVersion (repoRoot: string) : Result<ToolVersion, DevHostFailure> =
    ...
    if not (File.Exists path) then Error(MissingAuthorityFile path)
        ...
        try
            let text = File.ReadAllText path
            let m = Regex.Match(text, "^FROM\s+node:([0-9]+\.[0-9]+\.[0-9]+)", RegexOptions.Multiline)
            if not m.Success then Error(MalformedAuthorityFile path)
            else
                let raw = m.Groups.[1].Value
                match ToolVersion.parse raw with
...
let readElmPackageVersion (repoRoot: string) : Result<string, DevHostFailure> =
    ...
    if not (File.Exists path) then Error(MissingAuthorityFile path)
        ...
        try
            ...
            if not m.Success then Error(MalformedAuthorityFile path)
            else Ok m.Groups.[1].Value
        with _ -> Error(MalformedAuthorityFile path)

/// Pure helper used by the doctor to drive the dependency venv version.
let readPolicyPipVersion (manifestJson: string) : Result<ToolVersion, DevHostFailure> =
    let m = Regex.Match(manifestJson, "\"pip\"\s*:\s*\"([0-9]+\.[0-9]+(?:\.[0-9]+)?)\"")
    if not m.Success then Error(MalformedAuthorityFile "eng/devhost-toolchain.json")
    else
...
let readActionlintVersion (manifestJson: string) : Result<ToolVersion, DevHostFailure> =
    let m = Regex.Match(manifestJson, "\"actionlint\"\s*:.*?\"version\"\s*:\s*\"([0-9]+\.[0-9]+\.[0-9]+)\"")
    if not m.Success then Error(MalformedAuthorityFile "eng/devhost-toolchain.json")
    else
        match ToolVersion.parse m.Groups.[1].Value with
        | Ok v -> Ok v
...
let readShellCheckVersion (manifestJson: string) : Result<ToolVersion, DevHostFailure> =
    let m = Regex.Match(manifestJson, "\"shellcheck\"\s*:.*?\"version\"\s*:\s*\"([0-9]+\.[0-9]+\.[0-9]+)\"")
    if not m.Success then Error(MalformedAuthorityFile "eng/devhost-toolchain.json")
    else
        match ToolVersion.parse m.Groups.[1].Value with
        | Ok v -> Ok v
