...
type RealFilesystem() =
    ...
    interface IFilesystem with
        ...
        member _.ReadAllLines p = File.ReadAllLines p |> Array.toList
        member _.WriteAllText(p, c) = File.WriteAllText(p, c)
        member _.AppendAllText(p, c) = File.AppendAllText(p, c)
...
type IFakeFilesystem() =
    ...
    interface IFilesystem with
        ...
        member _.DeleteDir p = dirs <- Set.remove p dirs
        ...
        member _.Combine parts = String.concat "/" parts
...
type FakeEnvironment(initial: Map<string, string>) =
    ...
    interface IEnvironment with
        ...
        member _.SetEnv(key, value) = bag <- Map.add key value bag
...
type FakeConsole() =
    ...
    interface IConsole with
        member _.Stdout text = out <- text :: out
        member _.Stderr text = err <- text :: err
