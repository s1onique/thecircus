...
let private renderFailureSafe (status: CheckStatus) : string =
    match status with
    | Failed failure -> renderFailure failure
    | _ -> ""
...
let persist (fs: IFilesystem) (path: string) (report: EvidenceReport) : Result<string, DevHostFailure> =
    try
        let directory = System.IO.Path.GetDirectoryName path

        if not (System.String.IsNullOrEmpty directory) && not (fs.IsDirectory directory) then
