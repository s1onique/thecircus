...
module Probes =
                    ...
                    if eq > 0 then
                        ...
                        let value =
                            line.Substring(eq + 1).Trim()
                                .Trim('"')
                                .Replace("\\\"", "\"")
                        if key = "ID" then id <- value.ToLower()
                        elif key = "PRETTY_NAME" then pretty <- value
                if String.IsNullOrEmpty id then None
                else Some (id, pretty)
            with _ -> None
    ...
    let userId () : int =
        ...
        if System.String.IsNullOrEmpty raw then 0
        else
...
let classifyDistribution (id: string) : SupportedDistribution =
    ...
    | "ubuntu" -> Ubuntu
    | "debian" -> Debian
    | "linuxmint" | "linuxmintd" -> LinuxMint
    | _ -> OtherLinux
...
let isSupported (info: HostInfo) : Result<unit, DevHostFailure> =
    if info.Arch <> "AMD64" && info.Arch <> "x86_64" then
...
let probeHost () : HostInfo =
    let (id, pretty) =
        ...
        | None -> ("", "")
    let arch =
        if String.IsNullOrEmpty(Probes.architecture ()) then "unknown"
        else Probes.architecture ()
    {
        Arch = arch
        OsId = id
        PrettyName = pretty
