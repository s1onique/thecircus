...
type GitSeam =
    ...
    abstract member CheckAncestry:
        repoPath: string -> remoteOid: string -> localOid: string -> Result<bool, GitOperationFailure>
    ...
    interface GitSeam with
        member this.GetObjectFormat repoPath =
            ...
            | Ok output ->
                ...
                | "sha1" -> Ok Sha1
                | "sha256" -> Ok Sha256
                | unexpected -> Error(FormatDetectionFailed(sprintf "Unknown object format: '%s'" unexpected))
            | Error e -> Error(FormatDetectionFailed(sprintf "BoundedProcess failure: %A" e))

        member this.ResolveCommit repoPath oid =
            match
                BoundedProcess.runGitWithExit
                    repoPath
                    [ "rev-parse"; "--verify"; "--end-of-options"; sprintf "%s^{commit}" oid ]
                    BoundedProcess.defaultTimeoutMs
            with
            | Ok completion ->
                match completion.ExitCode with
                | 0 -> Ok()
                | other -> Error(CommitNotFound(oid, other, completion.Stderr))
            | Error e -> Error(OperationFailed(sprintf "BoundedProcess failure: %A" e))

        member this.ResolveObject repoPath oid =
            match
                BoundedProcess.runGitWithExit
                    repoPath
                    [ "cat-file"; "-e"; sprintf "%s^{object}" oid ]
                    BoundedProcess.defaultTimeoutMs
            with
            | Ok completion ->
                match completion.ExitCode with
                | 0 -> Ok()
                | other -> Error(ObjectNotFound(oid, other, completion.Stderr))
            | Error e -> Error(OperationFailed(sprintf "BoundedProcess failure: %A" e))
        ...
        member this.CheckAncestry repoPath remoteOid localOid =
            ...
            | Ok completion ->
                ...
                | other -> Error(AncestryCheckFailed(other, completion.Stderr))
            | Error e -> Error(OperationFailed(sprintf "BoundedProcess failure: %A" e))
...
let parsePrePushLine (format: ObjectFormat) (line: string) : Result<Types.PrePushRefUpdate, PrePushInputFailure> =
    ...
    if String.IsNullOrEmpty trimmed then
        ...
        if parts.Length <> 4 then
            ...
            let localRef, localOid, remoteRef, remoteOid =
            ...
            if not (isValidRef localRef) then
                ...
                if not localValid && not (isExactNullOid localOid expectedWidth) then
                    Error(
                        PrePushInputFailure.InvalidOid(
                            line,
                            localOid,
                            sprintf "must be %d hex chars or %d zeros" expectedWidth expectedWidth
                        )
                    )
                    ...
                    Ok
                        { Types.PrePushRefUpdate.LocalRef = localRef
...
let parsePrePushInput
    ...
    if Array.isEmpty lines then
        ...
        | Error e -> Error(FormatDetectionFailed(sprintf "Object format detection failed: %A" e))
        | Ok format ->
...
let isExistingTagUpdate (update: Types.PrePushRefUpdate) (format: ObjectFormat) =
    isTagRef update.RemoteRef
    && not (isExactNullOid update.RemoteOid format.OidWidth)
...
let verifyUpdate
    ...
    if not (isBranchRef update.RemoteRef || isTagRef update.RemoteRef) then
        ...
        isDeletion update objectFormat
    then
    ...
    // Handle new tag creation
        ...
        if isExactNullOid update.LocalOid objectFormat.OidWidth then
            ...
            | Error e -> Types.OperationalFailure(update, sprintf "object resolution failed: %A" e)
    // Handle existing branch update
        ...
        | Error e -> Types.OperationalFailure(update, sprintf "ancestry check failed: %A" e)
    else
