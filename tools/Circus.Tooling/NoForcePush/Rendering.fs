...
let renderDiagnosticHuman (d: Types.Diagnostic) : string =
    sprintf "[%s] %s:%d:%d %s: %s" d.RuleId d.Path d.Line d.Column d.RuleId d.Detail
...
let renderStaticPolicyHuman (result: Types.StaticPolicyResult) : string =
    ...
    if List.isEmpty result.Diagnostics then
        ...
        sb.AppendLine(sprintf "Violations (%d):" (List.length result.Diagnostics))
        |> ignore
...
let renderPrePushOutcomeHuman (outcome: Types.PrePushOutcome) : string =
    match outcome with
    | Types.Allowed update -> sprintf "ALLOWED %s: %s -> %s" update.RemoteRef update.RemoteOid update.LocalOid
