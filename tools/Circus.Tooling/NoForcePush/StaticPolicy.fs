...
let private forceOptionPattern =
    Regex(@"(?:\s|^)(--force|-f|-uf)(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect force-with-lease options.
let private forceWithLeasePattern =
    Regex(@"(?:\s|^)--force-with-lease(?:=\S+)?(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect force-if-includes options.
let private forceIfIncludesPattern =
    Regex(@"(?:\s|^)--force-if-includes(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect leading-plus refspec for force push.
let private leadingPlusRefspecPattern =
    Regex(@"(?:\s|^)\+\S+:refs/", RegexOptions.IgnoreCase)

/// Detect remote deletion options.
let private remoteDeletePattern =
    Regex(@"(?:\s|^)(--delete|-d)(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect empty-source deletion refspec (:refs/heads/x).
let private emptySourceDeletePattern =
    Regex(@"(?:\s|^):refs/", RegexOptions.IgnoreCase)

/// Detect mirror or prune options.
let private mirrorPrunePattern =
    Regex(@"(?:\s|^)--(?:mirror|prune)(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect no-verify option.
let private noVerifyPattern =
    Regex(@"(?:\s|^)--no-verify(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect send-pack invocation.
let private sendPackPattern =
    Regex(@"(?:\s|^)git\s+send-pack(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect hook bypass options.
let private hookBypassPattern =
    Regex(@"(?:\s|^)--(?:no-verify|verify)(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect dynamic push arguments (variables, $@, $*, etc.).
let private dynamicArgPattern =
    Regex(@"\$(?:[@*]|\{[^\}]+\}|[A-Za-z_][A-Za-z0-9_]*)", RegexOptions.IgnoreCase)

/// Detect eval or shell indirection.
let private evalPattern =
    Regex(@"(?:\s|^)(?:eval|exec|sh\s+-c)\s+", RegexOptions.IgnoreCase)

/// Detect GitHub API force options.
let private ghForcePattern =
    Regex(@"(?:\s|^)--field\s+force=true(?:\s|$)", RegexOptions.IgnoreCase)

/// Detect GitHub API ref deletion via gh.
let private ghDeleteRefPattern =
    Regex(@"gh\s+api\s+.*(?:ref|git)\s+.*(?:delete|remove)", RegexOptions.IgnoreCase)

/// Detect curl DELETE requests against Git ref endpoints.
let private curlDeleteRefPattern =
    Regex(@"curl\s+.*DELETE\s+.*(?:ref|git)(?:\s|$)", RegexOptions.IgnoreCase)
...
let containsRemoteDelete (command: string) : bool = remoteDeletePattern.IsMatch(command)
...
let containsMirrorOrPrune (command: string) : bool = mirrorPrunePattern.IsMatch(command)

/// Check if a command line contains no-verify.
let containsNoVerify (command: string) : bool = noVerifyPattern.IsMatch(command)

/// Check if a command line contains send-pack.
let containsSendPack (command: string) : bool = sendPackPattern.IsMatch(command)

/// Check if a command contains hook bypass.
let containsHookBypass (command: string) : bool = hookBypassPattern.IsMatch(command)
...
let containsGhForce (command: string) : bool = ghForcePattern.IsMatch(command)

/// Check if a command contains GitHub API ref deletion.
let containsGhDeleteRef (command: string) : bool = ghDeleteRefPattern.IsMatch(command)

/// Check if a command contains curl DELETE against Git ref.
let containsCurlDeleteRef (command: string) : bool = curlDeleteRefPattern.IsMatch(command)
...
let verifySurfaceFile (root: string) (entry: Types.SurfaceEntry) : Result<Types.Diagnostic list, Types.DiagnosticId> =
    try
        ...
        if not (File.Exists fullPath) then
            ...
            let commands =
                CommandLexer.extractCommandsFromContent entry.ParserKind content entry.Path
...
let verify (root: string) : Types.StaticPolicyResult =
    ...
    | Ok tracked ->
        ...
        | Ok entries ->
            ...
            | Error e -> errors.Add(sprintf "inventory validation failed: %A" e)
            | Ok() ->
                // Verify each surface
                for entry in entries do
                    ...
                    | Ok findings ->
                        for f in findings do
                            diagnostics.Add(f)
                    | Error e -> errors.Add(sprintf "%A" e)
