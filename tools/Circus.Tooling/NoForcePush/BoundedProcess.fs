...
let private readBoundedBytesSync
    ...
    let rec loop () =
        if ct.IsCancellationRequested || collected.Count >= limitBytes then
            ...
            let bytesRead =
                stream.Read(buffer, 0, min bufferSize (limitBytes - collected.Count))
...
let runBounded
    ...
    try
        ...
        if not exited then
            ...
            // Check results
            let stdoutResult =
                if stdoutTask.IsCompleted then
                    stdoutTask.Result
                else
                    Ok([||], 0)

            let stderrResult =
                if stderrTask.IsCompleted then
                    stderrTask.Result
                else
                    Ok([||], 0)
    ...
    | :? OperationCanceledException -> Error(TimeoutExceeded timeoutMs)
    | ex -> Error(ProcessLaunchFailed(sprintf "Unexpected error: %s" ex.Message))
...
let runGitCapture (repoPath: string) (args: string list) (timeoutMs: int) : Result<ProcessCompletion, BoundedFailure> =
...
let runGitQuery (repoPath: string) (args: string list) (timeoutMs: int) : Result<string, BoundedFailure> =
    ...
    | Ok completion ->
        if completion.ExitCode = 0 then
            ...
            Error(
                BoundedFailure.OutputReadFailed(sprintf "Git exited with %d: %s" completion.ExitCode completion.Stderr)
            )
    | Error e -> Error e

/// Run git expecting success, returning stdout.
/// Treats nonzero exit as failure.
let runGitExpectSuccess (repoPath: string) (args: string list) (timeoutMs: int) : Result<string, BoundedFailure> =
    runGitQuery repoPath args timeoutMs

/// Run git with full completion result.
/// Preserves all exit codes.
let runGitWithExit (repoPath: string) (args: string list) (timeoutMs: int) : Result<ProcessCompletion, BoundedFailure> =
...
let runGitWithExitCode (repoPath: string) (args: string list) (timeoutMs: int) : Result<int, BoundedFailure> =
    match runGitCapture repoPath args timeoutMs with
...
let runGit (repoPath: string) (args: string list) (timeoutMs: int) : Result<string * int, BoundedFailure> =
    match runGitCapture repoPath args timeoutMs with
