...
type OperationalFailure = { Operation: string; Detail: string }
...
let issueToString issue =
    ...
    | SubjectArgumentMismatch(payload, argument) ->
        sprintf "payload subject %s disagrees with --subject-commit %s" payload argument
    | SubjectTreeMismatch(payload, resolved) ->
        sprintf "payload subject tree %s disagrees with resolved tree %s" payload resolved
    ...
    | CommittedBlobMismatch(path, expected, actual) ->
        sprintf "committed blob mismatch for %s: expected=%s actual=%s" path expected actual
    | ReferencedHashMismatch(path, expected, actual) ->
        sprintf "SHA-256 mismatch for %s: expected=%s actual=%s" path expected actual
    | TranscriptSummaryMalformed detail -> "smoke transcript summary malformed: " + detail
    | TranscriptSummaryMismatch(declared, actual) ->
        sprintf "smoke payload/transcript summary mismatch: declared=%A actual=%A" declared actual
    | TranscriptScanMismatch(transcript, scan) ->
        sprintf "smoke transcript/scan mismatch: transcript=%A scan=%A" transcript scan
