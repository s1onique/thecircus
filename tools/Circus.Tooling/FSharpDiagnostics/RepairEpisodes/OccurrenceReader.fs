...
let private containsNul (bytes: byte[]) : bool = Array.exists (fun b -> b = 0uy) bytes
...
let private knownSpanNames: Set<string> =
    set [ "start_line"; "start_column"; "end_line"; "end_column" ]
...
let private failWith (failure: OccurrenceReadFailure) : 'a = raise (ParserAbort failure)

let private checkDuplicates (canonicalPath: string) (lineNumber: int) (fieldPrefix: string) (root: JsonElement) : unit =
...
let private checkUnknownFields
    ...
    let firstUnknown =
        ...
        |> Seq.tryPick (fun p -> if Set.contains p.Name allowed then None else Some p.Name)
...
let private requiredStringField (canonicalPath: string) (lineNumber: int) (field: string) (root: JsonElement) : string =
    let el =
...
let private requiredInt64Field (canonicalPath: string) (lineNumber: int) (field: string) (root: JsonElement) : int64 =
    let el =
        match tryGetProperty root field with
        | Some v -> v
        | None -> failWith (MissingField(canonicalPath, lineNumber, field))
...
let private parseSpan (canonicalPath: string) (lineNumber: int) (root: JsonElement) : SourceSpan =
    let el =
    ...
    if el.ValueKind = JsonValueKind.Null then
        ...
        let startColumn =
            requiredNullableIntField canonicalPath lineNumber "start_column" el
...
let private parseBuildContext (canonicalPath: string) (lineNumber: int) (root: JsonElement) : BuildContext option =
    let el =
    ...
    if el.ValueKind = JsonValueKind.Null then
        ...
        let projectContextId =
            requiredNullableIntField canonicalPath lineNumber "project_context_id" el
        ...
        let evaluationId =
            requiredNullableIntField canonicalPath lineNumber "evaluation_id" el

        let submissionId =
            requiredNullableIntField canonicalPath lineNumber "submission_id" el
...
let private parseLine (canonicalPath: string) (lineNumber: int) (line: string) : DiagnosticOccurrence =
    let doc =
        try
    ...
    let schemaVersion =
        requiredStringField canonicalPath lineNumber "schema_version" root
    ...
    let extractorVersion =
        requiredStringField canonicalPath lineNumber "extractor_version" root

    let captureId = requiredStringField canonicalPath lineNumber "capture_id" root

    let sourceKind =
        requiredEnumField canonicalPath lineNumber "source_kind" root tryParseSourceKind

    let eventOrdinal = requiredInt64Field canonicalPath lineNumber "event_ordinal" root

    let severity =
        requiredEnumField canonicalPath lineNumber "severity" root tryParseSeverity

    let subcategory =
        requiredNullableStringField canonicalPath lineNumber "subcategory" root
    ...
    let messageNormalized =
        requiredStringField canonicalPath lineNumber "message_normalized" root

    let locationKind =
        requiredEnumField canonicalPath lineNumber "location_kind" root tryParseLocationKind

    let sourcePath =
        requiredNullableStringField canonicalPath lineNumber "source_path" root

    let projectPath =
        requiredNullableStringField canonicalPath lineNumber "project_path" root

    let span = parseSpan canonicalPath lineNumber root

    let senderName =
        requiredNullableStringField canonicalPath lineNumber "sender_name" root

    let eventTimestamp =
        requiredNullableStringField canonicalPath lineNumber "event_timestamp" root

    let buildContext = parseBuildContext canonicalPath lineNumber root

    let legacyLineStart =
        requiredNullableIntField canonicalPath lineNumber "legacy_source_line_start" root

    let legacyLineEnd =
        requiredNullableIntField canonicalPath lineNumber "legacy_source_line_end" root
...
let private readBytes (canonicalPath: string) : Result<byte[], OccurrenceReadFailure> =
    ...
    with
    | :? IOException as ex -> Result.Error(FileReadFailed(canonicalPath, ex.Message))
    | :? UnauthorizedAccessException as ex -> Result.Error(FileReadFailed(canonicalPath, ex.Message))
        ...
        | Result.Ok bytes ->
            if containsNul bytes then
                ...
                try
                    ...
                    let mutable failure: OccurrenceReadFailure = FileMissing canonicalPath
                    ...
                    for line in lines do
                        ...
                        if not aborted then
                            if not (String.IsNullOrWhiteSpace line) then
                                try
                                    let occ = parseLine canonicalPath lineNumber line
                                    acc <- occ :: acc
                                with ParserAbort f ->
                                    aborted <- true
                                    failure <- f
