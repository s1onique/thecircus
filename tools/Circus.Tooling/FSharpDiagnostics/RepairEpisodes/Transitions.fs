...
let private countByFingerprint (occurrences: DiagnosticOccurrence list) : FingerprintCount list =
...
let linkSourceChange
    ...
    let resolveSource () : SourceLinkKind option =
        ...
        | Some p ->
            ...
            | None ->
                let declaredTouched =
                    declaredRelevant |> List.filter (fun d -> hasAnyChange changeEntries d)
    ...
    | None, Some(ProjectFileModified pp) -> buildSourceLink (ProjectFileModified pp) [ pp ] []
    | None, Some(DeclaredRelevantPathChanged paths) -> buildSourceLink (DeclaredRelevantPathChanged paths) paths []
...
let classifyAssessment
    ...
    let result =
        ...
        | PersistedSameCount ->
            if scopeOk then
                TransitionAssessment.ExactPersistence
            else
                ambiguity
        | PersistedCountDecreased ->
            if scopeOk then
                TransitionAssessment.MultiplicityImprovementCandidate
            else
                unassessable
        | PersistedCountIncreased ->
            if scopeOk then
                TransitionAssessment.MultiplicityRegressionCandidate
            else
                unassessable
...
let bumpKind (counts: TransitionCounts) (kind: ExactTransitionKind) : TransitionCounts =
    ...
    | PersistedSameCount ->
        { counts with
            PersistedSameCount = counts.PersistedSameCount + 1 }
    | PersistedCountDecreased ->
        { counts with
            PersistedCountDecreased = counts.PersistedCountDecreased + 1 }
    | PersistedCountIncreased ->
        { counts with
            PersistedCountIncreased = counts.PersistedCountIncreased + 1 }
    | EliminatedAfter ->
        { counts with
            EliminatedAfter = counts.EliminatedAfter + 1 }
    | IntroducedAfter ->
        { counts with
            IntroducedAfter = counts.IntroducedAfter + 1 }

let bumpAssessment (counts: TransitionCounts) (a: TransitionAssessment) : TransitionCounts =
    match a with
    | ObservedResolutionCandidate ->
        { counts with
            ResolutionCandidates = counts.ResolutionCandidates + 1 }
    | ObservedRegressionCandidate ->
        { counts with
            RegressionCandidates = counts.RegressionCandidates + 1 }
    | Unassessable ->
        { counts with
            Unassessable = counts.Unassessable + 1 }
    | _ -> counts
...
let buildTransitions
    ...
    let allFps =
        ...
        Set.union (Set.ofList bkeys) (Set.ofList akeys) |> Set.toList |> List.sort
    ...
    for fp in allFps do
        let b =
            (Map.tryFind fp beforeFps
             |> Option.map (fun f -> f.Count)
             |> Option.defaultValue 0)

        let a =
            (Map.tryFind fp afterFps
             |> Option.map (fun f -> f.Count)
             |> Option.defaultValue 0)
        ...
        let assessment =
            classifyAssessment kind compat sourceLink changeEntries meta.ProjectPath
