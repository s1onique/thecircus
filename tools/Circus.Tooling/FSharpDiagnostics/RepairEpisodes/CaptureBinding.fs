...
let private isValidCaptureId (captureId: string) : bool =
    if String.IsNullOrWhiteSpace captureId then
        false
    elif isAbsolute captureId then
        false
    elif captureId.Contains "/" || captureId.Contains "\\" then
        false
    elif captureId = "." || captureId = ".." then
        false
    elif captureId.IndexOf '\u0000' >= 0 then
        false
    else
        true
...
let private isValidRawArtifactPath (rawPath: string) : bool =
    if String.IsNullOrEmpty rawPath then
        false
    elif rawPath.Contains "\\" then
        false
    elif isAbsolute rawPath then
        false
    else
...
let private findReparsePointBetween (fileParent: string) (captureAbsDir: string) : string option =
...
let private artifactManifestDecoder: UTF8Encoding = UTF8Encoding(false, true)
...
let private artifactManifestContainsNul (bytes: byte[]) : bool = Array.exists (fun b -> b = 0uy) bytes
...
let private checkArtifactManifestDuplicates (path: string) (lineNumber: int) (root: JsonElement) : unit =
...
let private checkArtifactManifestUnknown (path: string) (lineNumber: int) (root: JsonElement) : unit =
    let firstUnknown =
        root.EnumerateObject()
        |> Seq.tryPick (fun p ->
            if Set.contains p.Name artifactManifestKnownFields then
                None
            else
                Some p.Name)
...
let private artifactManifestRequiredInt64 (path: string) (lineNumber: int) (field: string) (root: JsonElement) : int64 =
    let el =
        match tryGetProperty root field with
        | Some v -> v
        | None -> artifactManifestFail (MissingField(path, lineNumber, field))
...
let private parseArtifactManifestLine (path: string) (lineNumber: int) (line: string) : ArtifactManifestEntry =
    let doc =
        try
    ...
    let schemaVersion =
        artifactManifestRequiredString path lineNumber "schema_version" root
    ...
    let canonicalPath =
        artifactManifestRequiredString path lineNumber "canonical_path" root

    let originalPath =
        artifactManifestRequiredString path lineNumber "original_path" root

    let artifactClass =
        artifactManifestRequiredString path lineNumber "artifact_class" root
    ...
    let captureId =
        artifactManifestRequiredNullableString path lineNumber "capture_id" root

    let supersedes =
        artifactManifestRequiredNullableString path lineNumber "supersedes" root

    let supersededBy =
        artifactManifestRequiredNullableString path lineNumber "superseded_by" root

    let metadataGaps =
        artifactManifestRequiredStringList path lineNumber "metadata_gaps" root
                        ...
                        if not aborted then
                            if not (String.IsNullOrWhiteSpace line) then
                                try
                                    ...
                                    acc <- entry :: acc
                                with ArtifactManifestParseAbort f ->
                                    aborted <- true
                                    failure <- f
...
let private captureManifestDecoder: UTF8Encoding = UTF8Encoding(false, true)
...
let private captureManifestContainsNul (bytes: byte[]) : bool = Array.exists (fun b -> b = 0uy) bytes

let private checkCaptureManifestDuplicates (path: string) (lineNumber: int) (root: JsonElement) : unit =
    let seen = HashSet<string>(StringComparer.Ordinal)
    let mutable dup: string option = None
...
let private checkObjectAgainst (knownFields: Set<string>) (path: string) (lineNumber: int) (root: JsonElement) : unit =
    let firstUnknown =
        root.EnumerateObject()
        |> Seq.tryPick (fun p ->
            if Set.contains p.Name knownFields then
                None
            else
                Some p.Name)
...
let private captureManifestRequiredInt (path: string) (lineNumber: int) (field: string) (root: JsonElement) : int =
    let el =
        match tryGetProperty root field with
        | Some v -> v
        | None -> captureManifestFail (MissingField(path, lineNumber, field))
...
let private parseCaptureManifest (path: string) (lineNumber: int) (text: string) : CaptureManifest =
    let doc =
        try
    ...
    let schemaVersion =
        captureManifestRequiredString path lineNumber "schema_version" root
    ...
    let rawArtifacts =
        captureManifestRequiredStringList path lineNumber "raw_artifacts" root

    let command = captureManifestRequiredNullableString path lineNumber "command" root

    let workingDirectory =
        captureManifestRequiredNullableString path lineNumber "working_directory" root

    let repositoryCommitOid =
        captureManifestRequiredNullableString path lineNumber "repository_commit_oid" root

    let repositoryTreeOid =
        captureManifestRequiredNullableString path lineNumber "repository_tree_oid" root

    let workingTreeState =
        captureManifestRequiredNullableString path lineNumber "working_tree_state" root

    let sourceRootAliases =
        captureManifestRequiredAliasList path lineNumber "source_root_aliases" root

    let dotnetSdkVersion =
        captureManifestRequiredNullableString path lineNumber "dotnet_sdk_version" root

    let msbuildVersion =
        captureManifestRequiredNullableString path lineNumber "msbuild_version" root

    let fsharpCompilerVersion =
        captureManifestRequiredNullableString path lineNumber "fsharp_compiler_version" root

    let operatingSystem =
        captureManifestRequiredNullableString path lineNumber "operating_system" root

    let architecture =
        captureManifestRequiredNullableString path lineNumber "architecture" root

    let culture = captureManifestRequiredNullableString path lineNumber "culture" root

    let startedAt =
        captureManifestRequiredNullableString path lineNumber "started_at" root

    let completedAt =
        captureManifestRequiredNullableString path lineNumber "completed_at" root

    let exitCode = captureManifestRequiredNullableInt path lineNumber "exit_code" root

    let metadataGaps =
        captureManifestRequiredStringList path lineNumber "metadata_gaps" root
...
let private readCaptureManifestStrict (path: string) : Result<CaptureManifest, CaptureManifestReadFailure> =
    if not (File.Exists path) then
        Result.Error(FileMissing path)
    else
        let bytesResult =
        ...
        | Result.Ok bytes ->
            if captureManifestStartsWithBom bytes then
                ...
                | Result.Ok text ->
                    ...
                    with CaptureManifestParseAbort f ->
                        Result.Error f
...
let internal readCaptureManifest (path: string) : Result<CaptureManifest, CaptureManifestReadFailure> =
...
let private hashAndLength (path: string) : Result<int64 * string, CaptureBindingFailure> =
    ...
    | :? UnauthorizedAccessException as ex -> Result.Error(RawArtifactReadFailed(path, ex.Message))
...
let private bindRawArtifact
    ...
    if not (isValidRawArtifactPath rawPath) then
        ...
        let canonical = canonicalise (captureCanonicalDir captureId + "/" + rawPath)
                    ...
                    | None ->
                        if isReparsePoint fullPath then
                            ...
                            | Result.Ok(actualLength, actualHash) ->
                                let matches = manifest |> List.filter (fun e -> e.CanonicalPath = canonical)

                                match matches with
                                | [] -> Result.Error(ArtifactManifestEntryMissing canonical)
                                | _ :: _ :: _ -> Result.Error(ArtifactManifestEntryDuplicate canonical)
                                | [ entry ] ->
                                    if entry.CaptureId <> Some captureId then
                                        ...
                                        Result.Error(RawArtifactHashMismatch(canonical, entry.Sha256, actualHash))
                                    else
                                        Result.Ok
                                            { CanonicalPath = canonical
                                              ByteLength = actualLength
                                              Sha256 = actualHash }
                with PathInspectionFailed msg ->
...
let bindCapture (repoRoot: string) (request: CaptureBindingRequest) : Result<BoundCapture, CaptureBindingFailure> =
    ...
    if not (isValidCaptureId request.CaptureId) then
        ...
        | Result.Error(FileMissing _) -> Result.Error(CaptureManifestMissing captureManifestCanonical)
        ...
        | Result.Error(RootNotObject(_, line)) ->
            Result.Error(
                CaptureManifestReadFailed(captureManifestCanonical, sprintf "line %d: root is not an object" line)
            )
        | Result.Error(MissingField(_, line, field)) ->
            Result.Error(
                CaptureManifestReadFailed(captureManifestCanonical, sprintf "line %d: missing field '%s'" line field)
            )
        | Result.Error(DuplicateField(_, line, field)) ->
            Result.Error(
                CaptureManifestReadFailed(captureManifestCanonical, sprintf "line %d: duplicate field '%s'" line field)
            )
        | Result.Error(UnknownField(_, line, field)) ->
            Result.Error(
                CaptureManifestReadFailed(captureManifestCanonical, sprintf "line %d: unknown field '%s'" line field)
            )
        | Result.Error(WrongJsonKind(_, line, field)) ->
            Result.Error(
                CaptureManifestReadFailed(
                    captureManifestCanonical,
                    sprintf "line %d: wrong JSON kind for '%s'" line field
                )
            )
        | Result.Error(SchemaVersionMismatch(_, line, actual)) ->
            Result.Error(
                CaptureManifestReadFailed(
                    captureManifestCanonical,
                    sprintf "line %d: schema_version mismatch '%s'" line actual
                )
            )
        | Result.Ok manifest ->
                    ...
                    | Some _ ->
                        ...
                        | _ ->
                            ...
                            if List.isEmpty manifest.RawArtifacts then
                                ...
                                let artifactManifestPath = repoRelative repoRoot artifactsManifestCanonicalPath
                                ...
                                | Result.Error(ArtifactManifestReadFailure.NulBytePresent _) ->
                                    Result.Error(
                                        ArtifactManifestReadFailed(
                                            artifactsManifestCanonicalPath,
                                            0,
                                            "NUL byte is not canonical"
                                        )
                                    )
                                | Result.Error(ArtifactManifestReadFailure.InvalidJson(_, line, detail)) ->
                                    Result.Error(
                                        ArtifactManifestReadFailed(artifactsManifestCanonicalPath, line, detail)
                                    )
                                ...
                                | Result.Error(ArtifactManifestReadFailure.SchemaVersionMismatch(_, line, actual)) ->
                                    Result.Error(
                                        ArtifactManifestReadFailed(
                                            artifactsManifestCanonicalPath,
                                            line,
                                            "schema_version mismatch: " + actual
                                        )
                                    )
                                | Result.Ok artifactEntries ->
                                    ...
                                    for rawPath in manifest.RawArtifacts do
                                        if not aborted then
                                            match
                                                bindRawArtifact request.CaptureId rawPath artifactEntries seen repoRoot
                                            with
                                            | Result.Ok bound -> boundList <- bound :: boundList
                                            | Result.Error f ->
                                                aborted <- true
                                                boundFailure <- f
                                    ...
                                    if aborted then
                                        ...
                                        let occurrencesPath = repoRelative repoRoot occurrencesCanonicalPath

                                        match readOccurrences occurrencesPath with
                                        | Result.Error e -> Result.Error(OccurrenceStreamFailure e)
                                        | Result.Ok allOccurrences ->
                                            // 11. Select occurrences for this capture and
                                            //     reject duplicate event ordinals.
                                            let filtered =
                                                allOccurrences |> List.filter (fun o -> o.CaptureId = request.CaptureId)
                                            ...
                                            | Some n -> Result.Error(DuplicateOccurrenceOrdinal(request.CaptureId, n))
                                            | None ->
                                                let sorted = boundList |> List.sortBy (fun b -> b.CanonicalPath)
