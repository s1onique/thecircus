...
let parse (argv: string list) : Result<Command, string> =
    ...
    | "extract-binlog" :: _ -> Result.Error "extract-binlog requires a capture-id argument"
    | "extract-legacy-text" :: [ captureId ] -> Ok(ExtractLegacyTextCmd captureId)
    | "extract-legacy-text" :: _ -> Result.Error "extract-legacy-text requires a capture-id argument"
...
let private renderInventoryJson (repoRoot: string) : string =
    ...
    let canonicalEntries =
        ...
        |> List.map (fun d ->
            ...
            sprintf "{\"canonical_path\":\"%s\",\"byte_length\":%d,\"sha256\":\"%s\"}" safePath d.ByteLength d.Sha256)
        |> String.concat ","
    ...
    let scratchEntries =
        ...
        |> List.map (fun d ->
            let safePath = d.RelativePath.Replace('\\', '/')
            sprintf "{\"path\":\"%s\",\"byte_length\":%d,\"sha256\":\"%s\"}" safePath d.ByteLength d.Sha256)
        |> String.concat ","
...
let runInventory (repoRoot: string) (outputJson: bool) : int =
    let text =
        if outputJson then
            renderInventoryJson repoRoot
        else
            renderInventoryHuman repoRoot
...
let runExtractBinlog (repoRoot: string) (captureId: string) : int =
    ...
    if not (File.Exists manifestPath) then
        ...
        for raw in manifest.RawArtifacts do
            ...
            if binlogPath.IsNone && (extensionOf raw = ".binlog" || name = "build.binlog") then
        ...
        | Some p ->
            try
                let _, occs =
                    extractBinlog manifest.CaptureId manifest.SourceRootAliases ExtractorVersion p

                let lines = occs |> List.map renderOccurrence |> String.concat "\n"
                let outPath = repoRelative repoRoot (captureDir + "/binlog-occurrences.jsonl")
                writeLineOriented outPath lines

                stdout.WriteLine(
                    sprintf "fsharp-diagnostics extract-binlog: wrote %d occurrences to %s" (List.length occs) outPath
                )

                ExitCode.pass
            with BinlogExtractionFailure msg ->
...
let runExtractLegacyText (repoRoot: string) (captureId: string) : int =
    ...
    if not (File.Exists manifestPath) then
        ...
        if result.DiagnosticLookingUnparsedLines > 0 then
            stderr.WriteLine(
                sprintf "error: %d diagnostic-looking unparsed line(s)" result.DiagnosticLookingUnparsedLines
            )

            ExitCode.policyFailure
        elif not (List.isEmpty result.UndeclaredAbsolutePaths) then
            stderr.WriteLine(
                sprintf "error: undeclared absolute paths: %s" (String.concat ", " result.UndeclaredAbsolutePaths)
            )

            ExitCode.policyFailure
        else
            let lines = result.Occurrences |> List.map renderOccurrence |> String.concat "\n"
            let outPath = repoRelative repoRoot (captureDir + "/legacy-occurrences.jsonl")
            writeLineOriented outPath lines
...
let runRegenerate (repoRoot: string) : int =
    let summary, outcome, _legacy, binlogResults = runPipeline repoRoot None
    ...
    if not outcome.Success then
        ...
        if not (List.isEmpty binlogFailures) then
            stderr.WriteLine(sprintf "error: %d binlog capture(s) failed extraction" (List.length binlogFailures))
...
let runVerify (repoRoot: string) (outputJson: bool) : int =
    let summary, outcome, legacyResults, binlogResults = runPipeline repoRoot None
    ...
    let undeclared =
        legacyResults |> List.collect (fun (_, p) -> p.UndeclaredAbsolutePaths)

    let unparsed =
        legacyResults |> List.sumBy (fun (_, p) -> p.DiagnosticLookingUnparsedLines)
    ...
    let canonicalStr =
        if outcome.CanonicalByteIdenticalAfterFailure then
            "true"
        else
            "false"
...
let run (argv: string list) : int =
    ...
    | Ok RepairEpisodesInventory -> Circus.Tooling.FSharpDiagnostics.RepairEpisodes.Cli.run [ "inventory" ]
    | Ok RepairEpisodesRegenerate -> Circus.Tooling.FSharpDiagnostics.RepairEpisodes.Cli.run [ "regenerate" ]
    | Ok RepairEpisodesVerify -> Circus.Tooling.FSharpDiagnostics.RepairEpisodes.Cli.run [ "verify" ]
    | Ok(RepairEpisodesShow id) -> Circus.Tooling.FSharpDiagnostics.RepairEpisodes.Cli.run [ "show"; id ]
    | Ok RepairEpisodesHelp -> Circus.Tooling.FSharpDiagnostics.RepairEpisodes.Cli.run [ "help" ]
    | Ok RuleCandidatesRedirect
    | Ok RuleCandidatesInventory -> Circus.Tooling.FSharpDiagnostics.RuleCandidates.Cli.run [ "inventory" ]
    | Ok RuleCandidatesRegenerate -> Circus.Tooling.FSharpDiagnostics.RuleCandidates.Cli.run [ "regenerate" ]
    | Ok RuleCandidatesVerify -> Circus.Tooling.FSharpDiagnostics.RuleCandidates.Cli.run [ "verify" ]
    | Ok(RuleCandidatesShow id) -> Circus.Tooling.FSharpDiagnostics.RuleCandidates.Cli.run [ "show"; id ]
    | Ok RuleCandidatesHelp -> Circus.Tooling.FSharpDiagnostics.RuleCandidates.Cli.run [ "help" ]
    | Result.Error msg ->
        stderr.WriteLine(sprintf "error: %s" msg)
