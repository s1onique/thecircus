...
let private writeAndFlush (stagingDir: string) (f: PendingFile) : string =
    ...
    let body = if f.Body.EndsWith "\n" then f.Body else f.Body + "\n"
...
let private snapshotCanonicalHashes (canonicalDir: string) (files: PendingFile list) : Map<string, string> =
    ...
    |> List.map (fun f ->
        ...
        let hash = if File.Exists fullPath then sha256OfFile fullPath else ""
...
let publish
    ...
    let staging =
        ...
        Path.Combine(Path.GetDirectoryName canonicalDir, (Path.GetFileName canonicalDir) + ".staging." + guid)
    ...
    try
        ...
        let hashes =
            files |> List.map (fun f -> f.CanonicalFileName, writeAndFlush staging f)
    ...
    with ex ->
        ...
        if failClosed then
            { Success = false
              OutputHashes = []
              RetainedTempPaths = if Directory.Exists staging then [ staging ] else []
