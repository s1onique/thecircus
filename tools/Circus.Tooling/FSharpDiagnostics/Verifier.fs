...
let private extractLegacyFromCaptures
...
let private extractBinlogFromCaptures
    (captures: LoadedCapture list)
    (repoRoot: string)
    ...
    |> List.map (fun c ->
        ...
        | None -> c.CaptureRelativeDir, Result.Ok []
        | Some p ->
            try
...
let private renderLeafBodies
    ...
    : PendingFile list =
    let sortedOccs = occurrences |> List.sortBy (fun o -> o.CaptureId, o.EventOrdinal)
    let sortedFps = fingerprints |> List.sortBy (fun fp -> fp.Sha256)
    ...
    let occLines = sortedOccs |> List.map renderOccurrence |> String.concat "\n"
    ...
    let fpLines =
        ...
        header
        + "\n"
        + (sortedFps |> List.map renderFingerprintTsv |> String.concat "\n")
        ...
        let rows =
            ...
            |> List.map (fun (fp, o) ->
                let spanText =
                    ...
                    sprintf
                        "%s:%s-%s:%s"
                        (match s.StartLine with
                         | Some n -> string n
                         | None -> "")
                        (match s.StartColumn with
                         | Some n -> string n
                         | None -> "")
                        (match s.EndLine with
                         | Some n -> string n
                         | None -> "")
                        (match s.EndColumn with
                         | Some n -> string n
                         | None -> "")
    ...
    [ { CanonicalFileName = occurrencesFile
        Body = occLines }
      { CanonicalFileName = fingerprintsFile
        Body = fpLines }
      { CanonicalFileName = duplicatesFile
        Body = dupLines }
      // No migrations means zero records: publish exactly one LF byte.
      { CanonicalFileName = migrationMapFile
        Body = "" } ]
...
let buildCorpusSummary
    ...
    : CorpusSummary =
    let legacyUnparsed =
        legacy |> List.sumBy (fun (_, p) -> p.DiagnosticLookingUnparsedLines)
    ...
    let rawCount =
        artifactEntries |> List.filter (fun e -> e.ArtifactClass = "raw") |> List.length
    ...
    let metadataGaps =
        artifactEntries |> List.collect (fun e -> e.MetadataGaps) |> List.distinct
...
let runPipeline
    ...
    let legacyOccs = legacyResults |> List.collect (fun (_, p) -> p.Occurrences)
    ...
    let leafBodies = renderLeafBodies merged fps dupes
    ...
    let _ =
        ...
        |> List.map (fun f ->
            ...
            Circus.Tooling.FSharpDiagnostics.Serialization.writeLineOriented fullPath f.Body)
    ...
    let _ = writeLineOriented (Path.Combine(targetDir, summaryFile)) summaryBody
    ...
    let _ = writeLineOriented (Path.Combine(targetDir, summaryFile)) summaryBodyFinal
    ...
    let artifactEntriesFinal, _ =
        buildArtifactManifestEntries repoRoot captures discoveredDigestFinal
