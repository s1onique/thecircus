...
let resolveIdentity (repoRoot: string) : Result<ResolvedIdentity, IdentityFailure> =
    if String.IsNullOrWhiteSpace repoRoot then
        ...
        | Error err -> Result.Error(IdentityGitFailure(sprintf "object-format: %A" err))
        | Ok fmtRun ->
            if fmtRun.ExitCode <> 0 then
                ...
                | Some fmtStr ->
                    ...
                    | Error err -> Result.Error(IdentityGitFailure(sprintf "commit: %A" err))
                    | Ok commitRun ->
                        if commitRun.ExitCode <> 0 then
                            Result.Error(
                                IdentityGitFailure(sprintf "commit exit %d: %s" commitRun.ExitCode commitRun.Stderr)
                            )
                        else
                            ...
                            if not (isValidOid fmtStr commit) then
                                ...
                                // Step 3: resolve tree
                                match
                                    runGit repoRoot [ "rev-parse"; "--verify"; "--end-of-options"; "HEAD^{tree}" ]
                                with
                                | Error err -> Result.Error(IdentityGitFailure(sprintf "tree: %A" err))
                                | Ok treeRun ->
                                    if treeRun.ExitCode <> 0 then
                                        Result.Error(
                                            IdentityGitFailure(
                                                sprintf "tree exit %d: %s" treeRun.ExitCode treeRun.Stderr
                                            )
                                        )
                                    else
                                        ...
                                        if not (isValidOid fmtStr tree) then
                                            ...
                                            | Error err -> Result.Error(IdentityGitFailure(sprintf "status: %A" err))
                                            | Ok statusRun ->
...
let runAllChecks (defs: EvidenceCheckDefinition list) : EvidenceCheckResult list =
    defs |> List.map runCheck |> sortChecksDeterministic
...
let generate
    ...
    | Result.Ok identity ->
        ...
        | Ok scope ->
            let defs =
                CanonicalCheckDefinitions repoRoot scope.BaselineCommitOid scope.DeclarationPath identity.CommitOid
...
let verifyFailureToString (f: VerifyFailure) : string =
    ...
    | IdentityMismatch(field, expected, actual) ->
    ...
    if not (File.Exists path) then
        ...
        | Error msg ->
            { Path = path
              Evidence = None
              Validation = None
              Failure = Some(ReadFailed msg)
              BindingMatch = None }
        | Ok rawText when String.IsNullOrEmpty rawText ->
            { Path = path
              Evidence = None
              Validation = None
              Failure = Some(ReadFailed "empty file")
              BindingMatch = None }
        | Ok rawText ->
            ...
            | Result.Ok evidence ->
                ...
                if not (isValid vr) then
                    ...
                    let bindingResult =
                        match resolveIdentity repoRoot with
                        | Result.Error idErr -> Some(IdentityUnresolved(identityFailureToString idErr))
                        | Result.Ok current ->
...
type WorkingTreeState = { Dirty: bool }
...
type EvidenceFailure = { Reason: string; Detail: string }

let evidenceFailure (reason: string) (detail: string) : EvidenceFailure = { Reason = reason; Detail = detail }
...
type CanonicalEvidenceDependencies =
    { ResolveRepositoryIdentity: (string -> Result<RepositoryIdentity, EvidenceFailure>)
      ResolveScopeBinding: (string -> string -> string option -> string option -> Result<ScopeBinding, EvidenceFailure>)
...
let private productionResolveIdentity (repoRoot: string) : Result<RepositoryIdentity, EvidenceFailure> =
    ...
    | Result.Error err -> Result.Error(evidenceFailure "identity_failure" (identityFailureToString err))
...
let private productionResolveScope
    ...
    | Error error -> Error(evidenceFailure "scope_authority_failure" (errorToString error))
...
let private productionReadWorkingTree (repoRoot: string) : Result<WorkingTreeState, EvidenceFailure> =
    if String.IsNullOrWhiteSpace repoRoot || not (Directory.Exists repoRoot) then
        ...
        match runGit repoRoot [ "status"; "--porcelain=v1" ] with
        | Error err -> Result.Error(evidenceFailure "git_failure" (sprintf "status: %A" err))
        | Ok statusRun ->
            if statusRun.ExitCode <> 0 then
                Result.Error(
                    evidenceFailure "git_failure" (sprintf "status exit %d: %s" statusRun.ExitCode statusRun.Stderr)
                )
            else
...
let private productionRunCheck
    ...
    let result =
        run request cancellationToken |> Async.AwaitTask |> Async.RunSynchronously
...
let private productionReadArtifact (path: string) : Result<byte array, EvidenceFailure> =
    if not (File.Exists path) then
        ...
        with ex ->
            Result.Error(evidenceFailure "artifact_read_failed" (sprintf "%s: %s" (ex.GetType().Name) ex.Message))
...
let private productionWriteArtifact (path: string) (content: byte array) : Result<unit, EvidenceFailure> =
    let dir = Path.GetDirectoryName path
    ...
    | Ok tmp ->
        try
            ...
            Result.Ok()
        with ex ->
            Result.Error(evidenceFailure "artifact_write_failed" (sprintf "%s: %s" (ex.GetType().Name) ex.Message))
...
let private executeChecksWithDependencies
    ...
    if List.isEmpty defs then
        ...
        | None ->
            ...
            | None -> Result.Ok(sortChecksDeterministic acc)
    ...
    | Result.Ok identity ->
        ...
        | Result.Ok state ->
            if state.Dirty then
                ...
                match
                    deps.ResolveScopeBinding repoRoot identity.CommitOid scopeDeclarationPath (Some baselineCommit)
                with
                    ...
                    | Result.Ok scope ->
                        let mismatch =
                            if
                                not (
                                    String.Equals(
                                        scope.EvaluatedTreeOid,
                                        evidence.TestedTreeOid,
                            ...
                            then
                                ...
                                not (
                                    String.Equals(
                                        scope.PointerBlobOid,
                                        evidence.ActiveScopePointerBlobOid,
                                        StringComparison.OrdinalIgnoreCase
                                    )
                                )
                            then
                                ...
                                not (
                                    String.Equals(
                                        scope.DeclarationBlobOid,
                                        evidence.DeclarationBlobOid,
                                        StringComparison.OrdinalIgnoreCase
                                    )
                                )
                            then
...
let provideFailureToString (f: ProvideFailure) : string =
    ...
    | ProvideSubjectTreeMismatch(expected, actual) ->
        sprintf "subject tree mismatch: expected=%s actual=%s" expected actual
...
let resolveSubjectIdentity (repoRoot: string) (subjectOid: string) : Result<ResolvedIdentity, ProvideFailure> =
    if String.IsNullOrWhiteSpace subjectOid then
        Result.Error(ProvideInvalidSubjectOid "empty OID")
    else
        match runGit repoRoot [ "rev-parse"; "--show-object-format=storage" ] with
        | Error err -> Result.Error(ProvideIdentityFailure(IdentityGitFailure(sprintf "object-format: %A" err)))
        | Ok fmtRun ->
            if fmtRun.ExitCode <> 0 then
                Result.Error(
                    ProvideIdentityFailure(
                        IdentityGitFailure(sprintf "object-format exit %d: %s" fmtRun.ExitCode fmtRun.Stderr)
                    )
                )
            else
                let formatToken = fmtRun.Stdout.Trim()
                ...
                | Some fmtStr ->
                    ...
                    | Ok typeRun ->
                        if typeRun.ExitCode <> 0 then
                            ...
                            else if not (isValidOid fmtStr subjectOid) then
                                Result.Error(ProvideInvalidSubjectOid(sprintf "invalid %s OID: %s" fmtStr subjectOid))
                            else
                                match
                                    runGit
                                        repoRoot
                                        [ "rev-parse"; "--verify"; "--end-of-options"; subjectOid + "^{tree}" ]
                                with
                                | Error err ->
    ...
    if List.isEmpty defs then
        ...
        | None ->
            ...
            | None ->
                ...
                let sorted =
                    acc |> List.map (fun e -> e.Result.Id, e) |> List.sortBy fst |> List.map snd
    ...
    | Result.Ok identity ->
        ...
        | Result.Ok state ->
            if state.Dirty then
                ...
                match deps.ResolveScopeBinding repoRoot identity.CommitOid scopeDeclarationPath None with
                ...
                | Ok scope ->
                    ...
                    | Result.Ok executedChecks ->
                        ...
                        | Result.Error rpFailure -> Result.Error(ProvideRecordPipelineFailure rpFailure)
