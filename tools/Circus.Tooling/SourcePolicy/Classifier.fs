...
let classifyCategory (name: string) (ext: string) : FileCategory =
    if name = "go.mod" || name = "go.sum" then
    ...
    elif name.StartsWith("Makefile.") then
        FileCategory.Makefile
    elif ext = ".mk" then
        FileCategory.Makefile
    elif name = "Dockerfile" then
        FileCategory.Dockerfile
    elif name.StartsWith("Dockerfile.") then
        FileCategory.Dockerfile
    elif name = "docker-compose.yml" then
        FileCategory.Dockerfile
    elif name = "docker-compose.yaml" then
        FileCategory.Dockerfile
    elif name.StartsWith("docker-compose.") then
        FileCategory.Dockerfile
    elif name.StartsWith("compose.") then
        FileCategory.Dockerfile
    elif name = "compose.yml" then
        FileCategory.Dockerfile
    elif name = "compose.yaml" then
        FileCategory.Dockerfile
    elif name = "flake.nix" then
        FileCategory.NixFlake
    elif ext = ".md" then
        FileCategory.DeclarativeDoc
    elif ext = ".markdown" then
        FileCategory.DeclarativeDoc
    elif ext = ".json" then
        FileCategory.DeclarativeDoc
    elif ext = ".toml" then
...
let classifyLanguage (name: string) (ext: string) : SourceLanguage =
    if name = "go.mod" || name = "go.sum" then
        SourceLanguage.Go
    elif ext = ".fs" || ext = ".fsi" || ext = ".fsproj" || ext = ".fsx" then
        SourceLanguage.FSharp
    elif ext = ".elm" then
        SourceLanguage.Elm
    elif ext = ".sh" then
        SourceLanguage.PosixShell
    elif name = "Makefile" || name.StartsWith("Makefile.") || ext = ".mk" then
        SourceLanguage.Declarative
    elif name = "Dockerfile" || name.StartsWith("Dockerfile.") then
        SourceLanguage.Declarative
    elif
    ...
    then
        SourceLanguage.Declarative
    elif ext = ".py" || ext = ".pyw" then
        SourceLanguage.Python
    elif ext = ".go" then
        SourceLanguage.Go
    elif ext = ".js" || ext = ".cjs" || ext = ".mjs" || ext = ".jsx" then
        SourceLanguage.JavaScript
    elif ext = ".ts" || ext = ".cts" || ext = ".mts" || ext = ".tsx" then
        SourceLanguage.TypeScript
    elif ext = ".rb" then
        SourceLanguage.Ruby
    elif ext = ".pl" || ext = ".pm" then
        SourceLanguage.Perl
    elif ext = ".php" then
        SourceLanguage.Php
    elif ext = ".lua" then
        SourceLanguage.Lua
    elif ext = ".ps1" || ext = ".psm1" then
        SourceLanguage.PowerShell
    elif ext = ".hs" || ext = ".lhs" then
        SourceLanguage.Haskell
    elif ext = ".ml" || ext = ".mli" then
