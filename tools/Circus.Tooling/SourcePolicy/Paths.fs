...
let escapesRepository (repoRoot: string) (relativePath: string) : bool =
    ...
    if isAbsolute n then
        true
    elif n.Contains ".." then
        true
    else
        try
            ...
            let rootWithSep =
                if root.EndsWith(System.IO.Path.DirectorySeparatorChar.ToString()) then
                    root
                else
            ...
            if resolved = root then
                ...
                not (resolved.StartsWith(rootWithSep, System.StringComparison.Ordinal))
        with _ ->
            true
