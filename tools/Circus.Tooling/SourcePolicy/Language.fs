...
type FileCategory =
    ...
    member this.AsTag =
        match this with
        | FSharpProduction -> "fsharp-production"
        | FSharpScript -> "fsharp-script"
        | ElmSource -> "elm-source"
        | ShellScript -> "shell-script"
        | StageZeroLauncher -> "stage-zero-launcher"
        | Makefile -> "makefile"
        | Dockerfile -> "dockerfile"
        | WorkflowYaml -> "workflow-yaml"
        | NixFlake -> "nix-flake"
        | DeclarativeDoc -> "declarative-doc"
        | Unknown -> "unknown"

type ViolationCode =
    | ForbiddenSourceLanguage
    | ForbiddenInterpreterInvocation
    | OversizedShell
    | NonPosixShell
    | ShellContainsDomainLogic
    | UnknownExecutableShebang
    | UnregisteredExecutableText
    | BaselineMissing
    | BaselineMalformed
    | BaselineStale
    | BaselineDigestMismatch
    | BaselineMeasurementMismatch
    | BaselineExpansion
    ...
    member this.AsTag =
        ...
        | OversizedShell -> "oversized_shell"
        | NonPosixShell -> "non_posix_shell"
        ...
        | BaselineMalformed -> "baseline_malformed"
        | BaselineStale -> "baseline_stale"
        | BaselineDigestMismatch -> "baseline_digest_mismatch"
        | BaselineMeasurementMismatch -> "baseline_measurement_mismatch"
        | BaselineExpansion -> "baseline_expansion"
        | RepositoryBoundaryEscape -> "repository_boundary_escape"
        | FileReadFailure -> "file_read_failure"
        | GitInventoryFailure -> "git_inventory_failure"
...
let isApprovedElmExtension (ext: string) = ext = ".elm"

let isApprovedShellExtension (ext: string) = ext = ".sh"
