        ...
        if leading.Length > 0 && int leading.[0] = 0xFEFF then
            ...
            let shebangLine =
                if newlineIdx >= 0 then
                    leading.Substring(0, newlineIdx)
                else
                    leading
            ...
            if parts.Length = 0 then
                ...
                let rest =
                    if parts.Length > 1 then
                        parts |> Array.skip 1 |> Array.toList
                    else
                        []

                if head = "/bin/sh" then
                    ShebangPosixShell leading
                elif head = "/bin/bash" || head = "/usr/bin/bash" then
                    ShebangBash leading
                    ...
                    | first :: _ ->
                        if first = "sh" then
                            ...
                            || first = "go"
                        then
                            ShebangForbidden(first, leading)
                        else
                            ShebangUnknown leading
                    | [] -> ShebangUnknown leading
                elif
                    ...
                    || head = "go"
                then
                    ShebangForbidden(head, leading)
                elif head = "/usr/bin/sh" then
                    ShebangPosixShell leading
                else
                    ShebangUnknown leading
        else
