...
type AntiPattern =
    ...
    member this.AsTag =
        match this with
        | BashArray -> "bash-array"
        | BashDoubleBracket -> "bash-double-bracket"
        | BashSource -> "bash-source"
        | BashEval -> "bash-eval"
        | HeredocWithExec -> "heredoc-with-exec"
        ...
        | DownloadAndPipe -> "download-and-pipe"
        | DynConstruct -> "dynamic-construct"
        | DomainFunction -> "domain-function"

let private patterns: (AntiPattern * Regex) list =
    let re s =
        Regex(s, RegexOptions.Compiled ||| RegexOptions.Multiline)
...
let evaluate (path: string) (text: string) (shebang: ShebangClassification) : Finding list =
    ...
    | ShebangMissing ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                "shell script lacks a shebang"
                "shell/shebang-required"
                "#!/bin/sh"
                "<missing>"
        )
    | ShebangBomRejected _ ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                "shell script begins with a UTF-8 BOM"
                "shell/bom-rejected"
                "#!/bin/sh"
                "0xEF 0xBB 0xBF"
        )
    | ShebangBash _ ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                "shell script uses a Bash shebang"
                "shell/shebang-must-be-posix"
                "#!/bin/sh"
                "bash"
        )
    | ShebangEnv(i, _) ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                (sprintf "shell script uses /usr/bin/env %s" i)
                "shell/shebang-must-be-posix"
                "#!/bin/sh"
                (sprintf "/usr/bin/env %s" i)
        )
    | ShebangForbidden(i, _) ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                (sprintf "shell script uses forbidden interpreter %s" i)
                "shell/shebang-must-be-posix"
                "#!/bin/sh"
                i
        )
    | ShebangUnknown _ ->
        findings.Add(
            shebangFinding
                path
                UnknownExecutableShebang
                "shell script shebang is not a recognised interpreter"
                "shell/shebang-must-be-posix"
                "#!/bin/sh"
                "unrecognised"
        )
    ...
    | ShebangMissing ->
        findings.Add(
            shebangFinding
                path
                UnknownExecutableShebang
                "stage-zero launcher lacks a shebang"
                "stage-zero/shebang-required"
                "#!/bin/sh"
                "<missing>"
        )
    | _ ->
        findings.Add(
            shebangFinding
                path
                NonPosixShell
                "stage-zero launcher must use #!/bin/sh"
                "stage-zero/shebang-must-be-posix"
                "#!/bin/sh"
                (Shebang.tag shebang)
        )
