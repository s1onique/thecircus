...
let private recordSourceForbidden
    (findings: ResizeArray<Finding>)
    (path: string)
    (detail: string)
    (rule: string)
    (actual: string)
    ...
    findings.Add(
        { Path = path
          Code = ForbiddenSourceLanguage
          Line = None
          Detail = detail
          Rule = rule
          Expected = Some "F# or Elm"
          Actual = Some actual }
    )
...
let evaluateFile (cfg: VerifyConfig) (relativePath: string) : Domain.PerFile =
    let cls =
        try
            Classifier.classify cfg.RepoRoot relativePath
        with _ ->
            { Path = relativePath
    ...
    if isAbsolute relativePath then
        findings.Add(
            { Path = relativePath
              Code = RepositoryBoundaryEscape
              Line = None
              Detail = "inventory entry is an absolute path"
              Rule = "repository/no-absolute-paths"
              Expected = Some "repository-relative"
              Actual = Some "absolute" }
        )
    ...
    let shebang =
        Shebang.classify (safeRead cfg.RepoRoot relativePath |> Option.defaultValue "")

    match cls.Category with
    | FileCategory.FSharpProduction -> ()
    | FileCategory.FSharpScript ->
        recordSourceForbidden
            findings
            relativePath
            "F# script (.fsx) must live under tools/experiments/"
            "fsharp/fsx-only-in-examples"
            relativePath
    ...
    | FileCategory.ShellScript ->
        ...
        | None ->
            findings.Add(
                { Path = relativePath
                  Code = FileReadFailure
                  Line = None
                  Detail = "cannot read shell script"
                  Rule = "file/readable"
                  Expected = Some "readable"
                  Actual = Some "unreadable" }
            )
    ...
    | FileCategory.NixFlake
    | FileCategory.WorkflowYaml ->
        match safeRead cfg.RepoRoot relativePath with
    ...
    | FileCategory.Unknown ->
        ...
        | SourceLanguage.Python ->
            recordSourceForbidden
                findings
                relativePath
                "Python source is not an approved implementation language"
                "source/no-python"
                "Python"
        | SourceLanguage.Go ->
            recordSourceForbidden
                findings
                relativePath
                "Go source is not an approved implementation language"
                "source/no-go"
                "Go"
        | SourceLanguage.JavaScript ->
            recordSourceForbidden
                findings
                relativePath
                "JavaScript source is not an approved implementation language"
                "source/no-javascript"
                "JavaScript"
        | SourceLanguage.TypeScript ->
            recordSourceForbidden
                findings
                relativePath
                "TypeScript source is not an approved implementation language"
                "source/no-typescript"
                "TypeScript"
        | SourceLanguage.Ruby ->
            recordSourceForbidden
                findings
                relativePath
                "Ruby source is not an approved implementation language"
                "source/no-ruby"
                "Ruby"
        | SourceLanguage.Perl ->
            recordSourceForbidden
                findings
                relativePath
                "Perl source is not an approved implementation language"
                "source/no-perl"
                "Perl"
        | SourceLanguage.Php ->
            recordSourceForbidden
                findings
                relativePath
                "PHP source is not an approved implementation language"
                "source/no-php"
                "Php"
        | SourceLanguage.Lua ->
            recordSourceForbidden
                findings
                relativePath
                "Lua source is not an approved implementation language"
                "source/no-lua"
                "Lua"
        | SourceLanguage.PowerShell ->
            recordSourceForbidden
                findings
                relativePath
                "PowerShell source is not an approved implementation language"
        ...
        | SourceLanguage.Haskell ->
            recordSourceForbidden
                findings
                relativePath
                "Haskell source is not an approved implementation language"
                "source/no-haskell"
                "Haskell"
        | SourceLanguage.OCaml ->
            recordSourceForbidden
                findings
                relativePath
                "OCaml source is not an approved implementation language"
                "source/no-ocaml"
                "OCaml"
...
let verify (cfg: VerifyConfig) : VerificationOutcome =
    ...
    | InventoryFailed f ->
        { RepositoryRoot = cfg.RepoRoot
          ...
          Findings =
            [ { Path = "<repo>"
                Code = GitInventoryFailure
                Line = None
                Detail = Inventory.renderInventoryFailure f
                Rule = "git/ls-files"
                Expected = Some "ok"
                Actual = Some "failed" } ] }
    | InventoryEntries rawEntries ->
        ...
        | InventoryEntries entries ->
            ...
            let baselineEntries, baselineFindings =
                ...
                | Malformed detail ->
                    ...
                    [ { Path = cfg.BaselinePath
                        Code = BaselineMalformed
                        Line = None
                        Detail = detail
                        Rule = "baseline/parseable"
                        Expected = Some Baseline.Header
                        Actual = Some "malformed" } ]
            ...
            let perFile: Domain.PerFile list =
                relevant |> List.map (fun e -> evaluateFile cfg e.RelativePath)
                ...
                |> List.map (fun (pf: Domain.PerFile) ->
                    ...
                    if not hasOversized then
                        ...
                        match
                            Baseline.matchEntry
                                baselineEntries
                                pf.Classification.Path
                                pf.Classification.Sha256
                                pf.Classification.PhysicalLines
                        with
                        | MatchOk ->
                            { pf with
                                Findings = pf.Findings |> List.filter (fun f -> f.Code <> OversizedShell) }
                        | DigestMismatch(expected, actual) ->
                            { pf with
                                Findings =
                                    pf.Findings
                                    @ [ { Path = pf.Classification.Path
                                          Code = BaselineDigestMismatch
                                          Line = None
                                          Detail =
                                            sprintf "baseline digest mismatch (expected %s, got %s)" expected actual
                        ...
                        | MeasurementMismatch(expected, actual) ->
                            { pf with
                                Findings =
                                    pf.Findings
                                    @ [ { Path = pf.Classification.Path
                                          Code = BaselineMeasurementMismatch
                                          Line = None
                                          Detail =
                                            sprintf
                                                "baseline line-count mismatch (expected %d, got %d)"
                                                expected
                                                actual
            ...
            let present = entries |> List.map (fun e -> e.RelativePath) |> Set.ofList
            ...
            let expansionFindings =
                if cfg.RejectBaselineExpansion then
                    Baseline.expansionFindings baselineEntries allObserved
                else
                    []
