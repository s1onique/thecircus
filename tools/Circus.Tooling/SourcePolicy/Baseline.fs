...
let private parseCsvRow (line: string) : string list option =
    if String.IsNullOrEmpty line then
        ...
        cells <- (current.ToString()) :: cells
        if inQuotes then None else Some(List.rev cells)
...
let private validateRow (cells: string list) (lineNumber: int) : string option =
    ...
    | [ path; kind; linesStr; sha; owner; successor; reason ] ->
        if String.IsNullOrWhiteSpace path then
            ...
            || not (sha |> Seq.forall System.Char.IsLetterOrDigit)
        then
...
let load (repoRoot: string) : LoadResult =
    ...
    if not (File.Exists path) then
        ...
        try
            let lines =
                File.ReadAllLines path |> Array.filter (fun l -> not (String.IsNullOrEmpty l))

            if lines.Length = 0 then
                Malformed "baseline is empty"
            elif lines.[0] <> Header then
                Malformed "baseline header row is missing or incorrect"
            else
                let mutable entries = []
                        ...
                        | None ->
                            ...
                            | Some p when entryPath.CompareTo(p) <= 0 ->
                                sortError <-
                                    Some(
                                        sprintf
                                            "line %d: path '%s' is not greater than previous path '%s'"
                                            lineNumber
                                            entryPath
                                            p
                                    )
...
let matchEntry (entries: BaselineEntry list) (relativePath: string) (sha: string) (physicalLines: int) : Match =
    ...
    | Some entry ->
        if entry.Sha256 <> sha then
            DigestMismatch(entry.Sha256, sha)
        elif entry.PhysicalLines <> physicalLines then
            MeasurementMismatch(entry.PhysicalLines, physicalLines)
        else
            MatchOk
