...
let mutable internal InjectStartAsyncFailure: (unit -> string option) =
    fun () -> None

let mutable internal InjectStartAsyncAccessFailure: (unit -> exn option) =
    fun () -> None

let mutable internal InjectStartAsyncStdoutDrainFailure: (unit -> exn option) =
    fun () -> None

let mutable internal InjectStartAsyncObserverFailure: (unit -> exn option) =
    fun () -> None
...
let mutable internal ObserveStdoutDrainTask: (Task<Result<byte[], exn>> -> unit) =
    ignore

let mutable internal ObserveStderrDrainTask: (Task<Result<string, exn>> -> unit) =
    ignore
...
let mutable internal SubstituteStdoutDrainTask: (Task<Result<byte[], exn>> -> Task<Result<byte[], exn>>) =
    id

let mutable internal SubstituteStderrDrainTask: (Task<Result<string, exn>> -> Task<Result<string, exn>>) =
    id
...
let private inspectTerminal (task: Task<Result<'a, exn>>) (label: string) (note: string ref) : DrainSettleStatus =
    try
        if task.IsCanceled then
            ...
            let aggExes =
                if isNull task.Exception then
                    ...
                    seq { for e in task.Exception.InnerExceptions -> e }

            let detail = aggExes |> Seq.map (fun (e: exn) -> e.Message) |> String.concat "; "
        ...
        if task.IsCompleted then
            ...
            try
                if task.Wait(remaining) then
                    ...
                    appendNote
                        note
                        (sprintf "%s drain did not settle within shared %O deadline" label DrainSettleTimeout)
            ...
            | :? AggregateException as agg ->
                appendNote
                    note
                    (sprintf
                        "%s drain settle aggregate: %s"
                        label
                        (agg.InnerExceptions |> Seq.map (fun e -> e.Message) |> String.concat "; "))
...
let private settleDrainsShared
    ...
    let timedOutLabels =
        [ if stdoutStatus = TimeoutOccurred then
              yield "stdout"
          if stderrStatus = TimeoutOccurred then
              yield "stderr" ]
        ...
        while not finished do
            try
                ...
                if read <= 0 then
                    ...
                    ms.Write(buffer, 0, read) |> ignore
            with
            | :? OperationCanceledException -> finished <- true
            | ex ->
                firstError <- Some ex
                finished <- true
...
let private startAsync
    (argv: string list)
    (workingDir: string option)
    (ct: CancellationToken)
    (note: string ref)
    : Result<AsyncProcessContext, StartFailure> =
        ...
        | None ->
            ...
            if isNull proc then
                ...
                try
                    ...
                    let stdoutBytes =
                        SubstituteStdoutDrainTask(drainBytesAsync proc.StandardOutput.BaseStream ct)
                    ...
                    let stderrText =
                        SubstituteStderrDrainTask(drainTextAsync proc.StandardError.BaseStream ct)
                ...
                with ex ->
                    ...
                    let timeoutDetail = sprintf "%s AND cleanup settled with %A" detail settle
                    disposeProc proc note

                    match settle with
                    | AllDrainsTerminal -> Result.Error(ContextConstructionFailure detail)
                    | DrainTimeout labels -> Result.Error(ContextCleanupFailure(timeoutDetail, labels))

let private waitForExitAsync
    (ctx: AsyncProcessContext)
...
type private CleanupObservation = { Notes: string }
...
let private finalize
    ...
    if not stdoutOk then
        ...
        | Result.Error "cleanup_timeout" -> CleanupFailure(sprintf "bounded cleanup wait timed out: %s" note)
        | Result.Error other -> CleanupFailure(sprintf "%s: %s" other note)
...
let private runCore
    ...
    try
        try
            ...
            | Result.Error(PreSpawnFailure msg) -> bodyResult <- BodySpawnError msg
            | Result.Error(ContextConstructionFailure msg) -> bodyResult <- BodyUnexpected msg
            ...
            | Result.Ok ctx ->
                ...
                let verdict =
                    waitForExitAsync ctx cancellationToken cleanupNote |> Async.RunSynchronously
    ...
    let outcome =
        ...
        | BodyUnexpected msg ->
            ...
            | DrainTimeout labels ->
                CleanupFailure(
                    sprintf
                        "%s drain(s) timed out during cleanup: %s [%s]"
        ...
        | BodyCompleted(verdict, _stdoutRaw, _stdoutText, _stderrText, stdoutOk, stderrOk) ->
            ...
            | DrainTimeout labels ->
                CleanupFailure(
                    sprintf "%s drain(s) timed out during cleanup: %s" (String.concat ", " labels) cleanupNote.Value
                )
            | AllDrainsTerminal ->
