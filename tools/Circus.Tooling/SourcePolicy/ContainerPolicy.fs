...
let private requireFile (root: string) (relative: string) (id: string) : Violation list =
    if pathExists root relative then
        ...
        [ { Check = id
            Id = id
            Path = relative
            Detail = sprintf "required file missing: %s" relative } ]
...
let private requireExecutable (root: string) (relative: string) (id: string) : Violation list =
    if not (pathExists root relative) then
        ...
        [ { Check = id
            Id = id
            Path = relative
            Detail = sprintf "required shell script is not executable: %s" relative } ]
    else
        []
...
let private checkDockerignore (root: string) : Violation list =
    ...
    | None ->
        [ { Check = "CP-03_dockerignore"
            Id = "CP-03_dockerignore"
            Path = ".dockerignore"
            Detail = ".dockerignore is missing" } ]
    | Some text ->
            ...
            if not (Set.contains r actual) then
                violations <-
                    { Check = "CP-03_dockerignore"
                      Id = "CP-03_dockerignore"
                      Path = ".dockerignore"
                      Detail = sprintf ".dockerignore misses required exclusion: %s" r }
                    :: violations
...
let private checkWorkflowTriggers (root: string) : Violation list =
    ...
    for trigger in required do
        ...
        if not (Regex(pattern, RegexOptions.Multiline).IsMatch text) then
            violations <-
                { Check = "CP-04_workflow_triggers"
                  Id = "CP-04_workflow_triggers"
                  Path = ".github/workflows/harbor.yml"
                  Detail = sprintf "harbor.yml lacks trigger: %s" trigger }
                :: violations
...
let private checkPushBranchRestriction (root: string) : Violation list =
    ...
    if not (text.Contains "- main") then
        violations <-
            { Check = "CP-05_push_main"
              Id = "CP-05_push_main"
              Path = ".github/workflows/harbor.yml"
              Detail = "push publication is not restricted to main" }
            :: violations

    if not (text.Contains "v*") then
        violations <-
            { Check = "CP-05_push_tags"
              Id = "CP-05_push_tags"
              Path = ".github/workflows/harbor.yml"
              Detail = "version-tag publication trigger is missing" }
            :: violations
...
let private checkReusableInputs (root: string) : Violation list =
    ...
    for input in required do
        ...
        if not (Regex(pattern, RegexOptions.Multiline).IsMatch text) then
            violations <-
                { Check = "CP-08_reusable_inputs"
                  Id = "CP-08_reusable_inputs"
                  Path = ".github/workflows/harbor-build-image.yml"
                  Detail = sprintf "reusable workflow input missing: %s" input }
                :: violations

    if
        not (text.Contains "push:")
        || not (Regex("type:\s*boolean", RegexOptions.Multiline).IsMatch text)
    then
        violations <-
            { Check = "CP-08_reusable_push_type"
              Id = "CP-08_reusable_push_type"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "reusable push input must be boolean" }
            :: violations
...
let private checkNoPullRequestTarget (root: string) : Violation list =
    ...
    if topText.Contains "pull_request_target" then
        violations <-
            { Check = "CP-09_no_pull_request_target"
              Id = "CP-09_no_pull_request_target"
              Path = ".github/workflows/harbor.yml"
              Detail = "pull_request_target is forbidden" }
            :: violations
...
let private checkTrustedRunner (root: string) : Violation list =
    ...
    if not (topText.Contains "spbnix-k8s-docker") then
        violations <-
            { Check = "CP-10_trusted_runner"
              Id = "CP-10_trusted_runner"
              Path = ".github/workflows/harbor.yml"
              Detail = "trusted publication runner label is missing" }
            :: violations
...
let private checkHarborRepositoryNaming (root: string) : Violation list =
    ...
    if not (reusableText.Contains "harbor-pve1.spbnix.local/circus/${{ inputs.image_name }}") then
        violations <-
            { Check = "CP-11_harbor_naming"
              Id = "CP-11_harbor_naming"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "Harbor repository contract is missing" }
            :: violations

    for img in [ "circus-backend"; "circus-frontend" ] do
        if not (topText.Contains(sprintf "image_name: %s" img)) then
            violations <-
                { Check = "CP-11_harbor_image_contract"
                  Id = "CP-11_harbor_image_contract"
                  Path = ".github/workflows/harbor.yml"
                  Detail = sprintf "top-level image contract is missing: %s" img }
                :: violations
...
let private checkPasswordStdin (root: string) : Violation list =
    ...
    if
        (reusableText.Contains "HARBOR_PASSWORD")
        && reusableText.Contains "--password-stdin"
    then
        []
    else
...
let private checkTlsBypass (root: string) : Violation list =
    ...
    let forbidden =
        [ "--insecure"; "tls-verify=false"; "insecure=true"; "GODEBUG=x509ignoreCN=0" ]
    ...
    for marker in forbidden do
        if reusableText.Contains marker || caText.Contains marker then
            violations <-
                { Check = "CP-13_tls_bypass"
                  Id = "CP-13_tls_bypass"
                  Path = ".github/workflows/harbor-build-image.yml"
                  Detail = sprintf "TLS verification bypass is forbidden: %s" marker }
                :: violations
...
let private checkPrivateCaAndBuildkit (root: string) : Violation list =
    ...
    if not (caText.Contains "SPBNIX_CA_CERT_PEM") then
        violations <-
            { Check = "CP-14_ca_secret"
              Id = "CP-14_ca_secret"
              Path = ".github/scripts/install-spbnix-harbor-ca.sh"
              Detail = "CA script does not consume the CA secret" }
            :: violations

    if not (caText.Contains "buildkitd.toml") then
        violations <-
            { Check = "CP-14_buildkit_config"
              Id = "CP-14_buildkit_config"
              Path = ".github/scripts/install-spbnix-harbor-ca.sh"
              Detail = "CA script does not create BuildKit config" }
            :: violations

    if not (caText.Contains "[registry.\"${HARBOR_HOST}\"]") then
        violations <-
            { Check = "CP-14_buildkit_registry"
              Id = "CP-14_buildkit_registry"
              Path = ".github/scripts/install-spbnix-harbor-ca.sh"
              Detail = "BuildKit registry CA mapping is missing" }
            :: violations

    if not (reusableText.Contains "SPBNIX_CA_CERT_PEM") then
        violations <-
            { Check = "CP-14_reusable_ca"
              Id = "CP-14_reusable_ca"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "reusable workflow does not declare SPBNIX_CA_CERT_PEM" }
            :: violations
...
let private checkCacheSeparation (root: string) : Violation list =
    ...
    if not (reusableText.Contains cacheTemplate) then
        violations <-
            { Check = "CP-15_cache_template"
              Id = "CP-15_cache_template"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "Harbor cache reference template is missing" }
            :: violations
    ...
    for cacheName in [ "circus-backend"; "circus-frontend" ] do
        if not (topText.Contains(sprintf "cache_name: %s" cacheName)) then
            violations <-
                { Check = "CP-15_cache_image_specific"
                  Id = "CP-15_cache_image_specific"
                  Path = ".github/workflows/harbor.yml"
                  Detail = sprintf "image-specific cache reference is missing: %s" cacheName }
                :: violations
        else
            cacheRefs <-
                (sprintf "harbor-pve1.spbnix.local/circus/cache/%s:buildcache" cacheName)
                :: cacheRefs

    if List.length cacheRefs = 2 && cacheRefs.[0] = cacheRefs.[1] then
        violations <-
            { Check = "CP-15_cache_distinct"
              Id = "CP-15_cache_distinct"
              Path = ".github/workflows/harbor.yml"
              Detail = "backend and frontend cache references must remain distinct" }
            :: violations

    if
        not (List.isEmpty violations)
        && not (List.exists (fun v -> v.Id = "CP-15_cache_distinct") violations)
    then
        violations <-
            { Check = "CP-15_cache_distinct"
              Id = "CP-15_cache_distinct"
              Path = ".github/workflows/harbor.yml"
              Detail = "cache separation check detected contract violations" }
            :: violations
...
let private checkPublishGating (root: string) : Violation list =
    ...
    if not (buildText.Contains "PUBLISH") then
        violations <-
            { Check = "CP-16_build_publish_marker"
              Id = "CP-16_build_publish_marker"
              Path = "scripts/ci/build_image.sh"
              Detail = "build script does not gate on PUBLISH" }
            :: violations

    if not (publishText.Contains "PUBLISH") then
        violations <-
            { Check = "CP-16_publish_publish_marker"
              Id = "CP-16_publish_publish_marker"
              Path = "scripts/ci/publish_image.sh"
              Detail = "publish script does not gate on PUBLISH" }
            :: violations

    if not (buildText.Contains "== \"true\"") && not (buildText.Contains "!= \"true\"") then
        violations <-
            { Check = "CP-16_build_compare"
              Id = "CP-16_build_compare"
              Path = "scripts/ci/build_image.sh"
              Detail = "build script does not compare PUBLISH to 'true'" }
            :: violations

    if
        not (publishText.Contains "== \"true\"")
        && not (publishText.Contains "!= \"true\"")
    then
        violations <-
            { Check = "CP-16_publish_compare"
              Id = "CP-16_publish_compare"
              Path = "scripts/ci/publish_image.sh"
              Detail = "publish script does not compare PUBLISH to 'true'" }
            :: violations

    if not (reusableText.Contains "PUBLISH: ${{ steps.metadata.outputs.publish }}") then
        violations <-
            { Check = "CP-16_reusable_publish_forward"
              Id = "CP-16_reusable_publish_forward"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "reusable workflow must forward the PUBLISH env to the build/publish scripts" }
            :: violations

    if
        not (List.isEmpty violations)
        && not (List.exists (fun v -> v.Id = "CP-16_publish_gating") violations)
    then
        violations <-
            { Check = "CP-16_publish_gating"
              Id = "CP-16_publish_gating"
              Path = "scripts/ci/build_image.sh"
              Detail = "publish-gating check detected contract violations" }
            :: violations
...
let private checkCacheImportExport (root: string) : Violation list =
    ...
    if not (buildText.Contains "cache-from") then
        violations <-
            { Check = "CP-17_cache_from"
              Id = "CP-17_cache_from"
              Path = "scripts/ci/build_image.sh"
              Detail = "registry cache import is missing" }
            :: violations

    if not (publishText.Contains "cache-to") then
        violations <-
            { Check = "CP-17_cache_to"
              Id = "CP-17_cache_to"
              Path = "scripts/ci/publish_image.sh"
              Detail = "registry cache export is missing" }
            :: violations

    if not (publishText.Contains "mode=max") then
        violations <-
            { Check = "CP-17_cache_mode_max"
              Id = "CP-17_cache_mode_max"
              Path = "scripts/ci/publish_image.sh"
              Detail = "cache mode=max is missing" }
            :: violations

    if not (publishText.Contains "oci-mediatypes=true,image-manifest=true") then
        violations <-
            { Check = "CP-17_cache_oci_manifest"
              Id = "CP-17_cache_oci_manifest"
              Path = "scripts/ci/publish_image.sh"
              Detail = "Harbor cache export is missing the OCI image-manifest compatibility options" }
            :: violations

    if
        not (List.isEmpty violations)
        && not (List.exists (fun v -> v.Id = "CP-17_cache_import_export") violations)
    then
        violations <-
            { Check = "CP-17_cache_import_export"
              Id = "CP-17_cache_import_export"
              Path = "scripts/ci/build_image.sh"
              Detail = "cache import/export check detected contract violations" }
            :: violations
...
let private checkImmutableTags (root: string) : Violation list =
    ...
    if not (metadata.Contains "GITHUB_SHA") || not (metadata.Contains "local-${sha}") then
        violations <-
            { Check = "CP-18_immutable_tag"
              Id = "CP-18_immutable_tag"
              Path = ".github/scripts/harbor-metadata.sh"
              Detail = "full commit SHA is not part of the immutable tag contract" }
            :: violations
    ...
    for tag in releaseTags do
        if not (metadata.Contains tag) then
            violations <-
                { Check = "CP-18_release_tag"
                  Id = "CP-18_release_tag"
                  Path = ".github/scripts/harbor-metadata.sh"
                  Detail = sprintf "release tag missing: %s" tag }
                :: violations

    if not (reusableText.Contains "steps.metadata.outputs.publish == 'true'") then
        violations <-
            { Check = "CP-18_trusted_guard"
              Id = "CP-18_trusted_guard"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "trusted publication steps are not guarded by metadata publication" }
            :: violations
...
let private checkLatestTagContract (root: string) : Violation list =
    ...
    if not (metadata.Contains "latest") then
        violations <-
            { Check = "CP-19_latest_present"
              Id = "CP-19_latest_present"
              Path = ".github/scripts/harbor-metadata.sh"
              Detail = "latest tag is not present" }
            :: violations

    if not (metadata.Contains "refs/heads/main") then
        violations <-
            { Check = "CP-19_latest_main_only"
              Id = "CP-19_latest_main_only"
              Path = ".github/scripts/harbor-metadata.sh"
              Detail = "latest tag is not gated to refs/heads/main" }
            :: violations

    if Regex.Matches(metadata, "latest").Count <> 1 then
        violations <-
            { Check = "CP-19_latest_unique"
              Id = "CP-19_latest_unique"
              Path = ".github/scripts/harbor-metadata.sh"
              Detail = "latest must be emitted only once by the metadata contract" }
            :: violations

    if
        not (List.isEmpty violations)
        && not (List.exists (fun v -> v.Id = "CP-19_latest_main_only") violations)
    then
        violations <-
            { Check = "CP-19_latest_main_only"
              Id = "CP-19_latest_main_only"
              Path = ".github/scripts/harbor-metadata.sh"
              Detail = "latest-tag contract check detected contract violations" }
            :: violations
...
let private checkSecretMountCleanup (root: string) : Violation list =
    ...
    for marker in required do
        if not (frontend.Contains marker) then
            violations <-
                { Check = "CP-20_secret_marker"
                  Id = "CP-20_secret_marker"
                  Path = "Dockerfile.frontend"
                  Detail = sprintf "frontend Dockerfile is missing the required CA marker: %s" marker }
                :: violations

    if frontend.Contains "update-ca-certificates" then
        violations <-
            { Check = "CP-20_update_ca"
              Id = "CP-20_update_ca"
              Path = "Dockerfile.frontend"
              Detail = "frontend Dockerfile mutates the system CA store" }
            :: violations

    if frontend.Contains "/tmp/spbnix-ca.crt" then
        violations <-
            { Check = "CP-20_legacy_path"
              Id = "CP-20_legacy_path"
              Path = "Dockerfile.frontend"
              Detail = "frontend Dockerfile references the obsolete /tmp/spbnix-ca.crt path" }
            :: violations
...
let private checkElmInstaller (root: string) : Violation list =
    ...
    for marker in markers do
        if not (frontend.Contains marker) then
            violations <-
                { Check = "CP-21_elm_marker"
                  Id = "CP-21_elm_marker"
                  Path = "Dockerfile.frontend"
                  Detail = sprintf "frontend Dockerfile is missing the required Elm installer marker: %s" marker }
                :: violations

    if not (Regex("0\.19\.2|Elm\\s0\.19\.2").IsMatch frontend) then
        violations <-
            { Check = "CP-21_elm_version"
              Id = "CP-21_elm_version"
              Path = "Dockerfile.frontend"
              Detail = "frontend Dockerfile does not assert Elm 0.19.2" }
            :: violations
...
let private checkNumericUsers (root: string) : Violation list =
    ...
    if not (Regex("USER\\s+\\d+:\\d+").IsMatch backend) then
        violations <-
            { Check = "CP-22_backend_user"
              Id = "CP-22_backend_user"
              Path = "Dockerfile.backend"
              Detail = "backend final image lacks an explicit numeric non-root user" }
            :: violations

    if not (Regex("USER\\s+\\d+:\\d+").IsMatch frontend) then
        violations <-
            { Check = "CP-22_frontend_user"
              Id = "CP-22_frontend_user"
              Path = "Dockerfile.frontend"
              Detail = "frontend final image lacks an explicit numeric non-root user" }
            :: violations
...
let private checkPortContracts (root: string) : Violation list =
    ...
    if
        not (backend.Contains "ASPNETCORE_HTTP_PORTS=8080")
        || not (backend.Contains "EXPOSE 8080")
    then
        violations <-
            { Check = "CP-23_backend_port"
              Id = "CP-23_backend_port"
              Path = "Dockerfile.backend"
              Detail = "backend 8080 contract is incomplete" }
            :: violations

    if not (nginx.Contains "listen 8080") || not (frontend.Contains "EXPOSE 8080") then
        violations <-
            { Check = "CP-23_frontend_port"
              Id = "CP-23_frontend_port"
              Path = "Dockerfile.frontend"
              Detail = "frontend 8080 contract is incomplete" }
            :: violations
...
let private checkSmokeEndpoints (root: string) : Violation list =
    ...
    if
        not (smoke.Contains "/health/live")
        && not (smoke.Contains "GET /health/live")
        && not (smoke.Contains "health/live")
    then
        violations <-
            { Check = "CP-24_backend_smoke"
              Id = "CP-24_backend_smoke"
              Path = "scripts/container-smoke.sh"
              Detail = "backend smoke test does not check /health/live" }
            :: violations

    for marker in [ "/healthz"; "<title>The Circus</title>"; "GET /" ] do
        if not (smoke.Contains marker) then
            violations <-
                { Check = "CP-24_frontend_smoke"
                  Id = "CP-24_frontend_smoke"
                  Path = "scripts/container-smoke.sh"
                  Detail = sprintf "frontend smoke contract is missing: %s" marker }
                :: violations
...
let private checkDigestPullInspect (root: string) : Violation list =
    ...
    if not (verifyText.Contains "docker pull \"${IMAGE_REPOSITORY}@${digest}\"") then
        violations <-
            { Check = "CP-25_digest_pull"
              Id = "CP-25_digest_pull"
              Path = "scripts/ci/verify_build_image.sh"
              Detail = "post-push verification does not pull by registry digest" }
            :: violations

    if not (verifyText.Contains "docker image inspect \"${IMAGE_REPOSITORY}@${digest}\"") then
        violations <-
            { Check = "CP-25_digest_inspect"
              Id = "CP-25_digest_inspect"
              Path = "scripts/ci/verify_build_image.sh"
              Detail = "post-push verification does not inspect the digest reference" }
            :: violations

    if
        not (reusableText.Contains "linux/amd64")
        || not (verifyPubText.Contains "architecture")
    then
        violations <-
            { Check = "CP-25_amd64_verify"
              Id = "CP-25_amd64_verify"
              Path = ".github/workflows/harbor-build-image.yml"
              Detail = "amd64 post-publication verification is incomplete" }
            :: violations
...
let private checkWorkflowSeams (root: string) : Violation list =
    ...
    let seams =
        [ ("Create configured BuildKit builder", "PUBLISH: ${{ steps.metadata.outputs.publish }}")
          ...
          ("Publish tested revision with provenance and SBOM attempt",
    ...
    for (stepName, snippet) in seams do
        if not (reusableText.Contains(sprintf "name: %s" stepName)) then
            violations <-
                { Check = "CP-26_seam_step"
                  Id = "CP-26_seam_step"
                  Path = ".github/workflows/harbor-build-image.yml"
                  Detail = sprintf "reusable workflow is missing the %s step" stepName }
                :: violations
        elif not (reusableText.Contains snippet) then
            violations <-
                { Check = "CP-26_seam_forward"
                  Id = "CP-26_seam_forward"
                  Path = ".github/workflows/harbor-build-image.yml"
                  Detail = sprintf "reusable workflow step '%s' does not forward %s" stepName snippet }
                :: violations
...
let private checkGithubOutputContracts (root: string) : Violation list =
    ...
    for (scriptText, label) in
        [ (buildText, "scripts/ci/build_image.sh")
          ...
          (verifyText, "scripts/ci/verify_build_image.sh") ] do
        if not (scriptText.Contains "GITHUB_OUTPUT") then
            violations <-
                { Check = "CP-27_github_output"
                  Id = "CP-27_github_output"
                  Path = label
                  Detail = sprintf "%s must use $GITHUB_OUTPUT for step outputs" label }
                :: violations

    for outputRef in
        [ "steps.builder.outputs.builder"
          "steps.builder.outputs.driver"
          "steps.remote_verify.outputs.digest" ] do
        if not (reusableText.Contains outputRef) then
            violations <-
                { Check = "CP-27_workflow_output"
                  Id = "CP-27_workflow_output"
                  Path = ".github/workflows/harbor-build-image.yml"
                  Detail = sprintf "reusable workflow must reference ${{ %s }}" outputRef }
                :: violations
...
let private checkActionPins (root: string) : Violation list =
    ...
    for m in Regex.Matches(combined, "^\s*uses:\s*(\S.*?)\s*$", RegexOptions.Multiline) do
        ...
        if raw <> "" && not (raw.StartsWith "./") then
            ...
            if lastAt < 0 then
                violations <-
                    { Check = "CP-28_action_pin"
                      Id = "CP-28_action_pin"
                      Path = "workflows"
                      Detail = sprintf "uses entry is missing @ pin: %s" raw }
                    :: violations
            else
                ...
                if not allowed then
                    violations <-
                        { Check = "CP-28_action_allowlist"
                          Id = "CP-28_action_allowlist"
                          Path = "workflows"
                          Detail = sprintf "unapproved external action: %s" action }
                        :: violations

                if not (Regex("^[0-9a-f]{40}$").IsMatch revision) then
                    violations <-
                        { Check = "CP-28_action_sha_pin"
                          Id = "CP-28_action_sha_pin"
                          Path = "workflows"
                          Detail = sprintf "uses entry is not pinned to a full SHA: %s" raw }
                        :: violations
    ...
    if not foundExternal then
        violations <-
            { Check = "CP-28_action_present"
              Id = "CP-28_action_present"
              Path = "workflows"
              Detail = "reusable workflows must depend on actions/checkout (no third-party checkouts detected)" }
            :: violations
...
let private checkTrackedSecrets (root: string) : Violation list =
    ...
    | TrackedInventoryFailed f ->
        [ { Check = "CP-29_tracked_secrets"
            ...
            Detail =
              sprintf
                  "git ls-files failed (cannot prove the secret scan is complete): %s"
                  (Inventory.renderInventoryFailure f) } ]
    | TrackedFiles tracked ->
        let secretPattern =
            Regex("(^|/)\\.env(\\.|$)|.*\\.(pem|key|p12|pfx|dockerconfigjson)$", RegexOptions.IgnoreCase)
...
let private checkFinalStageExclusions (root: string) : Violation list =
    ...
    for f in forbidden do
        if finalStage.Contains f then
            violations <-
                { Check = "CP-30_final_stage_material"
                  Id = "CP-30_final_stage_material"
                  Path = "Dockerfile.frontend"
                  Detail = sprintf "frontend final image mentions forbidden runtime material: %s" f }
                :: violations
...
let private checkGateSummaryAcceptance (root: string) : Violation list =
    ...
    for marker in required do
        if not (acceptanceText.Contains marker) then
            violations <-
                { Check = "CP-31_acceptance_marker"
                  Id = "CP-31_acceptance_marker"
                  Path = "tests/ci/test_gate_summary_acceptance.sh"
                  Detail = sprintf "gate-summary acceptance test is missing required marker: %s" marker }
                :: violations

    for vocab in [ "pass"; "fail"; "skip"; "unavailable" ] do
        if not (acceptanceText.Contains vocab) then
            violations <-
                { Check = "CP-31_acceptance_vocab"
                  Id = "CP-31_acceptance_vocab"
                  Path = "tests/ci/test_gate_summary_acceptance.sh"
                  Detail = sprintf "gate-summary acceptance test must reference canonical status: %s" vocab }
                :: violations

    let shellTest = readText root "tests/ci/test_build_publish_shell.sh"

    if
        not (shellTest.Contains "PUBLISH=true")
        || not (shellTest.Contains "PUBLISH=false")
    then
        violations <-
            { Check = "CP-31_publish_branch_coverage"
              Id = "CP-31_publish_branch_coverage"
              Path = "tests/ci/test_build_publish_shell.sh"
              Detail = "shell test suite does not exercise both PUBLISH branches" }
            :: violations

    if not (shellTest.Contains "wire_buildx_builder.sh") then
        violations <-
            { Check = "CP-31_wire_coverage"
              Id = "CP-31_wire_coverage"
              Path = "tests/ci/test_build_publish_shell.sh"
              Detail = "shell test suite does not exercise wire_buildx_builder.sh" }
            :: violations

    if not (shellTest.Contains "GITHUB_OUTPUT") then
        violations <-
            { Check = "CP-31_github_output_assertion"
              Id = "CP-31_github_output_assertion"
              Path = "tests/ci/test_build_publish_shell.sh"
              Detail = "shell test suite does not assert GITHUB_OUTPUT is used for step outputs" }
            :: violations
...
let private checks: CheckDefinition list =
    [ { CheckId = "CP-01_required_files"
        ImplementationFunctionName = nameof checkRequiredFiles
        Run = checkRequiredFiles }
      { CheckId = "CP-02_shell_executable"
        ImplementationFunctionName = nameof checkShellExecutable
        Run = checkShellExecutable }
      { CheckId = "CP-03_dockerignore"
        ImplementationFunctionName = nameof checkDockerignore
        Run = checkDockerignore }
      { CheckId = "CP-04_workflow_triggers"
        ImplementationFunctionName = nameof checkWorkflowTriggers
        Run = checkWorkflowTriggers }
      { CheckId = "CP-05_push_main"
        ImplementationFunctionName = nameof checkPushBranchRestriction
        Run = checkPushBranchRestriction }
      { CheckId = "CP-06_minimal_permissions"
        ImplementationFunctionName = nameof checkMinimalPermissions
        Run = checkMinimalPermissions }
      { CheckId = "CP-07_concurrency"
        ImplementationFunctionName = nameof checkReferenceScopedConcurrency
        Run = checkReferenceScopedConcurrency }
      { CheckId = "CP-08_reusable_inputs"
        ImplementationFunctionName = nameof checkReusableInputs
        Run = checkReusableInputs }
      { CheckId = "CP-09_no_pull_request_target"
        ImplementationFunctionName = nameof checkNoPullRequestTarget
        Run = checkNoPullRequestTarget }
      { CheckId = "CP-10_trusted_runner"
        ImplementationFunctionName = nameof checkTrustedRunner
        Run = checkTrustedRunner }
      { CheckId = "CP-11_harbor_naming"
        ImplementationFunctionName = nameof checkHarborRepositoryNaming
        Run = checkHarborRepositoryNaming }
      { CheckId = "CP-12_password_stdin"
        ImplementationFunctionName = nameof checkPasswordStdin
        Run = checkPasswordStdin }
      { CheckId = "CP-13_tls_bypass"
        ImplementationFunctionName = nameof checkTlsBypass
        Run = checkTlsBypass }
      { CheckId = "CP-14_ca_secret"
        ImplementationFunctionName = nameof checkPrivateCaAndBuildkit
        Run = checkPrivateCaAndBuildkit }
      { CheckId = "CP-15_cache_distinct"
        ImplementationFunctionName = nameof checkCacheSeparation
        Run = checkCacheSeparation }
      { CheckId = "CP-16_publish_gating"
        ImplementationFunctionName = nameof checkPublishGating
        Run = checkPublishGating }
      { CheckId = "CP-17_cache_import_export"
        ImplementationFunctionName = nameof checkCacheImportExport
        Run = checkCacheImportExport }
      { CheckId = "CP-18_immutable_tag"
        ImplementationFunctionName = nameof checkImmutableTags
        Run = checkImmutableTags }
      { CheckId = "CP-19_latest_main_only"
        ImplementationFunctionName = nameof checkLatestTagContract
        Run = checkLatestTagContract }
      { CheckId = "CP-20_secret_marker"
        ImplementationFunctionName = nameof checkSecretMountCleanup
        Run = checkSecretMountCleanup }
      { CheckId = "CP-21_elm_marker"
        ImplementationFunctionName = nameof checkElmInstaller
        Run = checkElmInstaller }
      { CheckId = "CP-22_backend_user"
        ImplementationFunctionName = nameof checkNumericUsers
        Run = checkNumericUsers }
      { CheckId = "CP-23_backend_port"
        ImplementationFunctionName = nameof checkPortContracts
        Run = checkPortContracts }
      { CheckId = "CP-24_backend_smoke"
        ImplementationFunctionName = nameof checkSmokeEndpoints
        Run = checkSmokeEndpoints }
      { CheckId = "CP-25_digest_pull"
        ImplementationFunctionName = nameof checkDigestPullInspect
        Run = checkDigestPullInspect }
      { CheckId = "CP-26_seam_step"
        ImplementationFunctionName = nameof checkWorkflowSeams
        Run = checkWorkflowSeams }
      { CheckId = "CP-27_github_output"
        ImplementationFunctionName = nameof checkGithubOutputContracts
        Run = checkGithubOutputContracts }
      { CheckId = "CP-28_action_pin"
        ImplementationFunctionName = nameof checkActionPins
        Run = checkActionPins }
      { CheckId = "CP-29_tracked_secrets"
        ImplementationFunctionName = nameof checkTrackedSecrets
        Run = checkTrackedSecrets }
      { CheckId = "CP-30_final_stage_material"
        ImplementationFunctionName = nameof checkFinalStageExclusions
        Run = checkFinalStageExclusions }
      { CheckId = "CP-31_acceptance_marker"
        ImplementationFunctionName = nameof checkGateSummaryAcceptance
        Run = checkGateSummaryAcceptance } ]
...
let CheckMetadataEntries: CheckMetadataEntry list =
    ...
    |> List.map (fun c ->
        { CheckId = c.CheckId
          ImplementationFunctionName = c.ImplementationFunctionName })
...
let runCheckById (id: string) (root: string) : Violation list =
    ...
    | None ->
        [ { Check = id
            Id = id
            Path = "<unknown>"
            Detail = sprintf "unknown check id: %s" id } ]
...
let verify (root: string) : ContainerPolicyReport =
    ...
    for c in checks do
        ...
        if List.isEmpty v then
            ...
            if
                c.CheckId = "CP-29_tracked_secrets"
                && List.exists (fun x -> x.Detail.Contains "git ls-files failed") v
            then
                operational <-
                    "CP-29 git inventory failed (cannot prove the secret scan is complete)"
                    :: operational
...
let runVerify (root: string) : int =
    try
        ...
        if not (List.isEmpty report.OperationalFailures) then
            stderr.WriteLine(
                sprintf "container-policy verify: FAIL (operational: %d)" (List.length report.OperationalFailures)
            )
            ...
            stderr.WriteLine(
                sprintf
                    "container-policy verify: FAIL (checks=%d, failed=%d, violations=%d)"
                    report.ChecksTotal
                    report.ChecksFailed
