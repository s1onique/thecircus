...
let private decodeStrict
    (recordBytes: byte[])
    (recordIndex: int)
    (commandId: string)
    (recordStartOffset: int)
    : Result<string, DecodeDiagnostic> =
    ...
    if recordBytes.Length = 0 then
        ...
        | :? DecoderFallbackException as ex ->
            let localOffset =
                if ex.Index >= 0 && ex.Index < recordBytes.Length then
                    ex.Index
                else
                    0
...
let parse (commandId: string) (bytes: byte[]) : ParseResult =
    if bytes.Length = 0 then
        ...
        if finalLen > 0 then
            ...
            let hasRealRecord = records |> Seq.exists (fun (rb, _) -> rb.Length > 0)
...
let renderDiagnostic (d: DecodeDiagnostic) : string =
    let categoryText =
        ...
        | InvalidUtf8(b, _) -> sprintf "invalid_utf8 byte=0x%02x" b
        | UnterminatedFinalRecord n -> sprintf "unterminated_final_record length=%d" n
        | EmptyInteriorRecord i -> sprintf "empty_interior_record index=%d" i

    let safeHex =
        if d.SafeBytesHex = "" then
            ""
        else
            sprintf " hex=%s" (sanitiseAscii d.SafeBytesHex)

    sprintf "command=%s record=%d offset=%d category=%s%s" d.CommandId d.RecordIndex d.ByteOffset categoryText safeHex
